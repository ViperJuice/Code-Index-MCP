...
### Phase 78 — Legacy Codex Phase-Loop Rebound Tail Recovery (SEMCODEXLOOPREBOUNDTAIL)
...
- [ ] A refreshed repo-local force-full rerun on the post-SEMQUERYFULLTAIL
      head either advances durably beyond the re-exposed
      `.codex/phase-loop/.../launch.json ->
      .codex/phase-loop/.../terminal-summary.json`
      pair or emits a truthful newer blocker before the 120-second watchdog
      expires.
- [ ] The chosen repair for the legacy `.codex/phase-loop` rebound seam stays
      narrow, tested, and does not reopen the repaired
      `scripts/run_comprehensive_query_test.py ->
      scripts/index_all_repos_semantic_full.py`
      boundary without direct evidence.
- [ ] `docs/status/SEMANTIC_DOGFOOD_REBUILD.md` records the
      SEMQUERYFULLTAIL rerun outcome and the final live verdict for the
      re-exposed legacy `.codex/phase-loop` pair.

**Scope notes**

This phase exists only if SEMQUERYFULLTAIL proves the later
...
- No reopening of unrelated semantic-stage, script-family, or release work
...
### Phase 88 — Legacy Codex Phase-Loop Rebound Relapse Tail Recovery (SEMCODEXLOOPREBOUNDRELAPSETAIL)

**Objective**

Carry the live force-full rerun beyond the re-exposed earlier legacy
`.codex/phase-loop` rebound seam revealed after
...
- [ ] A refreshed repo-local force-full rerun on the
      ...
      beyond the re-exposed
      `.codex/phase-loop/runs/20260424T190651Z-01-garc-plan/launch.json ->
      .codex/phase-loop/runs/20260427T075236Z-05-idxsafe-repair/terminal-summary.json`
      pair or emits a truthful newer blocker before the 120-second watchdog
      expires.
- [ ] Any repair chosen for this re-exposed earlier legacy
      `.codex/phase-loop` rebound seam stays narrow, tested, keeps canonical
      `.phase-loop/` authoritative, and does not reopen the cleared
      `.codex/phase-loop/runs/20260425T051448Z-01-garecut-execute/launch.json ->
      .codex/phase-loop/runs/20260425T051448Z-01-garecut-execute/heartbeat.json`
      seam without direct evidence.
- [ ] `docs/status/SEMANTIC_DOGFOOD_REBUILD.md` records the
      SEMCODEXLOOPGARECUTHEARTBEATTAIL rerun outcome and the final live
      verdict for the re-exposed
      `garc-plan launch.json -> idxsafe-repair terminal-summary.json` pair.

**Scope notes**

This phase exists only if SEMCODEXLOOPGARECUTHEARTBEATTAIL proves the exact
legacy heartbeat seam
`.codex/phase-loop/runs/20260425T051448Z-01-garecut-execute/launch.json ->
.codex/phase-loop/runs/20260425T051448Z-01-garecut-execute/heartbeat.json`
is not the final active blocker, but the refreshed live rerun on the new head
still regresses earlier in lexical walking to the re-exposed legacy rebound
pair
`.codex/phase-loop/runs/20260424T190651Z-01-garc-plan/launch.json ->
.codex/phase-loop/runs/20260427T075236Z-05-idxsafe-repair/terminal-summary.json`.
...
- No reopening of the cleared `garecut-execute launch.json -> heartbeat.json`
  seam once the live rerun has already moved away from it.
- No broad reopening of canonical `.phase-loop/` runtime behavior unless the
  refreshed rerun proves the active blocker cannot be cleared with a narrower
  legacy `.codex/phase-loop` rebound repair.
...
### Phase 90 — Legacy Codex Phase-Loop GAREL Heartbeat Rebound Tail Recovery (SEMCODEXLOOPGARELHEARTBEATREBOUNDTAIL)

**Objective**

Carry the live force-full rerun beyond the newer legacy `.codex/phase-loop`
compatibility-runtime seam revealed after SEMCODEXLOOPCIFLOWEXECRELAPSETAIL:
...
- [ ] A refreshed repo-local force-full rerun on the
      ...
      beyond `.codex/phase-loop/runs/20260424T225641Z-01-garel-execute/heartbeat.json ->
      .codex/phase-loop/runs/20260425T022006Z-01-garecut-plan/launch.json`
      or emits a truthful newer blocker before the 120-second watchdog
      expires.
- [ ] Any repair chosen for this later legacy `.codex/phase-loop`
      compatibility-runtime seam stays narrow, tested, keeps canonical
      `.phase-loop/` authoritative, and does not reopen the cleared
      `.codex/phase-loop/runs/20260427T081704Z-09-ciflow-execute/terminal-summary.json ->
      .codex/phase-loop/runs/20260427T081704Z-09-ciflow-execute/launch.json`
      seam without direct evidence.
- [ ] `docs/status/SEMANTIC_DOGFOOD_REBUILD.md` records the
      ...
      verdict for the exact `garel-execute heartbeat.json -> garecut-plan
      launch.json` pair.

**Scope notes**

This phase exists only if SEMCODEXLOOPCIFLOWEXECRELAPSETAIL proves the newer
legacy execute relapse pair
`.codex/phase-loop/runs/20260427T081704Z-09-ciflow-execute/terminal-summary.json ->
.codex/phase-loop/runs/20260427T081704Z-09-ciflow-execute/launch.json`
is not the final active blocker, but the refreshed live rerun on the new head
still terminalizes later in lexical walking on the newer legacy
...
- No reopening of the cleared `ciflow-execute terminal-summary.json ->
  launch.json` seam once the live rerun has already moved away from it.
- No broad reopening of canonical `.phase-loop/` runtime behavior unless the
  refreshed rerun proves the active blocker cannot be cleared with a narrower
  legacy `.codex/phase-loop` compatibility-runtime repair.
- No reopening of unrelated integration, script-family, security, or semantic
...
**Produces**
- IF-0-SEMCODEXLOOPGARELHEARTBEATREBOUNDTAIL-1 — later legacy
...
- SEMCODEXLOOPRELAPSEREBOUNDTAIL should amend the roadmap immediately if the
  ...
  `.codex/phase-loop/runs/20260425T051448Z-01-garecut-execute/launch.json ->
  .codex/phase-loop/runs/20260425T051448Z-01-garecut-execute/heartbeat.json`.
- SEMCODEXLOOPGARECUTHEARTBEATTAIL should amend the roadmap immediately if the
  live rerun proves the `garecut-execute launch.json -> heartbeat.json` seam
  is no longer the final active blocker but re-exposes an earlier exact legacy
...
- SEMCODEXLOOPREBOUNDRELAPSETAIL should amend the roadmap immediately if the
  ...
  `.codex/phase-loop/runs/20260427T081704Z-09-ciflow-execute/terminal-summary.json ->
  .codex/phase-loop/runs/20260427T081704Z-09-ciflow-execute/launch.json`.
- SEMCODEXLOOPCIFLOWEXECRELAPSETAIL should amend the roadmap immediately if
  the live rerun proves the `ciflow-execute terminal-summary.json ->
  launch.json` seam is no longer the final active blocker but exposes a later
...
- SEMCODEXLOOPTAIL should amend the roadmap immediately if the refreshed live
  ...
  `tests/docs/test_semincr_contract.py ->
  tests/docs/test_gabase_ga_readiness_contract.py`.
- SEMQUERYFULLTAIL should amend the roadmap immediately if the refreshed live
  rerun clears
  `scripts/run_comprehensive_query_test.py ->
  scripts/index_all_repos_semantic_full.py`
