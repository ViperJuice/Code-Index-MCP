...
This guide documents the beta Docker path for Code-Index-MCP `1.2.0-rc5`.
...
indexed automatically. Indexed MCP results are authoritative only when readiness
is `ready`; unavailable indexes return `index_unavailable` with
`safe_fallback: "native_search"`.

## Table of Contents

1. [Overview](#overview)
2. [Image Contract](#image-contract)
...
The MCP Index Server image provides a containerized solution for code indexing
without requiring local Python installation. Language support varies by plugin,
sandbox path, and optional dependencies; see [SUPPORT_MATRIX.md](SUPPORT_MATRIX.md)
for the canonical support contract.

### Why Docker?

- **Containerized setup**: Docker supplies the Python runtime and package install
...
than relying on documented variant tags. Semantic search requires provider
configuration such as `VOYAGE_API_KEY`; artifact sync requires GitHub CLI
authentication and suitable repository permissions.
...
2. **Pull the image**:
   ```bash
   docker pull ghcr.io/viperjuice/code-index-mcp:v1.2.0-rc5
   ```

3. **Create launcher script**:
   ```bash
   # Linux/macOS
   echo 'docker run -i --rm -v $(pwd):/workspace ghcr.io/viperjuice/code-index-mcp:v1.2.0-rc5 "$@"' > /usr/local/bin/mcp-index
...
`MCP_ALLOWED_ROOTS`, register each repo, and inspect readiness before MCP tool
use:
...
Register one worktree per git common directory. Same-repo sibling worktrees and
non-default branch queries return `index_unavailable` with
`safe_fallback: "native_search"` instead of reusing another checkout's index.
...
services:
  mcp-index:
    image: ghcr.io/viperjuice/code-index-mcp:v1.2.0-rc5
    volumes:
      - ./:/workspace
...
```bash
# Check sync status
docker run -it -v $(pwd):/workspace ghcr.io/viperjuice/code-index-mcp:v1.2.0-rc5 artifact status

# Download latest index
docker run -it -v $(pwd):/workspace ghcr.io/viperjuice/code-index-mcp:v1.2.0-rc5 artifact pull

# Upload your index
docker run -it -v $(pwd):/workspace ghcr.io/viperjuice/code-index-mcp:v1.2.0-rc5 artifact push
```
...
docker run -it -v /mnt/c/path/to/code:/workspace ghcr.io/viperjuice/code-index-mcp:v1.2.0-rc5
```
...
2. **Use WSL paths** for volume mounts:
   ...
   docker run -it -v /mnt/c/Users/name/project:/workspace ghcr.io/viperjuice/code-index-mcp:v1.2.0-rc5
   ```
...
echo '{"jsonrpc":"2.0","method":"initialize","id":1}' | \
  docker run -i --rm ghcr.io/viperjuice/code-index-mcp:v1.2.0-rc5
...
docker run --rm ghcr.io/viperjuice/code-index-mcp:v1.2.0-rc5 --health

# Test database connection
docker run --rm -v $(pwd):/workspace ghcr.io/viperjuice/code-index-mcp:v1.2.0-rc5 test db

# Verify index status
docker run --rm -v $(pwd):/workspace ghcr.io/viperjuice/code-index-mcp:v1.2.0-rc5 index status
```
...
```dockerfile
FROM ghcr.io/viperjuice/code-index-mcp:v1.2.0-rc5
