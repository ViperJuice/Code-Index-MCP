...
def test_document_processing():
    ...
    try:
        ...
        plaintext_plugin = PlainTextPlugin(
            language_config=plaintext_config,
            sqlite_store=None,
        ...
        md_ext = getattr(markdown_plugin, 'extensions', markdown_plugin._get_supported_extensions() if hasattr(markdown_plugin, '_get_supported_extensions') else [])
        txt_ext = getattr(plaintext_plugin, 'extensions', plaintext_plugin._get_supported_extensions() if hasattr(plaintext_plugin, '_get_supported_extensions') else [])
    ...
    for filename, (expected_md, expected_txt) in test_files.items():
        ...
        if md_supports == expected_md and txt_supports == expected_txt:
            ...
            print(f"  ❌ {filename}: Expected MD={expected_md}, TXT={expected_txt}, Got MD={md_supports}, TXT={txt_supports}")
            all_correct = False
        ...
        if isinstance(result, dict):
            ...
            if symbols:
                for i, symbol in enumerate(symbols[:5]):
                    print(f"    - {symbol.get('name', 'unnamed')} ({symbol.get('type', 'unknown')}) at line {symbol.get('start_line', '?')}")
