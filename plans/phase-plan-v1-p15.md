...
- **Spec↔runbook scope discrepancy resolved.** Spec line 780 lists 4 required GitHub token scopes (`contents:read`, `actions:read+write`, `attestations:write`). Runbook §2 adds `metadata:read` as a one-time P12 prereq. SL-4's validator enforces all five cumulatively. SL-docs.3 appends a post-execution amendment to spec §Phase 15 noting the `metadata:read` addition.
