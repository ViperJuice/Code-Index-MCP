...
phase: SEMMIXEDPHASETAIL
roadmap: specs/phase-plans-v7.md
roadmap_sha256: 29d2e7cb58ac1c64a5cb22cd301c5cb1519a888af002b0e22009c7faa85ee75e
---
# SEMMIXEDPHASETAIL: Mixed-Version Phase Plan Tail Recovery

## Context

SEMMIXEDPHASETAIL is the phase-53 follow-up for the v7 semantic hardening
...
`SEMMIXEDPHASETAIL` as the current `unplanned` phase after
`SEMLEGACYPLANS` closed out on `HEAD` `0064f6c5523d` with verification
`passed`, a clean worktree, and `main...origin/main [ahead 101]`. Legacy
...
- `specs/phase-plans-v7.md` is tracked and clean in this worktree, and
  ...
  `29d2e7cb58ac1c64a5cb22cd301c5cb1519a888af002b0e22009c7faa85ee75e`.
- The target artifact `plans/phase-plan-v7-SEMMIXEDPHASETAIL.md` did not
  exist before this planning run.
- `docs/status/SEMANTIC_DOGFOOD_REBUILD.md` is the active evidence anchor.
  Its SEMLEGACYPLANS live-rerun block captures the current downstream
  lexical failure precisely: at evidence time `2026-04-29T16:29:12Z`, the
  ...
  /home/viperjuice/code/Code-Index-MCP/plans/phase-plan-v7-SEMPHASETAIL.md`,
  and `In-flight path:
  /home/viperjuice/code/Code-Index-MCP/plans/phase-plan-v5-gagov.md`.
- The SEMLEGACYPLANS target pair is no longer the active blocker:
  `plans/phase-plan-v6-WATCH.md ->
  plans/phase-plan-v1-p19.md`. The live rerun now reaches the mixed-version
  phase-plan pair
  `plans/phase-plan-v7-SEMPHASETAIL.md ->
  plans/phase-plan-v5-gagov.md`, so older downstream assumptions should be
  treated as stale.
- The newly exposed pair is a mixed-version but still modern Markdown seam
  rather than a no-frontmatter historical outlier.
  `plans/phase-plan-v7-SEMPHASETAIL.md` is `375` lines / `22077` bytes and
  `plans/phase-plan-v5-gagov.md` is `277` lines / `16560` bytes. Both
  carry phase-loop frontmatter plus long `Context`, `Interface Freeze
  Gates`, `Lanes`, `Verification`, `Acceptance Criteria`, and automation
  sections, but `GAGOV` still references legacy `.codex/phase-loop/`
  runner state in its prose.
- The Markdown plugin already contains a generic bounded-name contract that
  ...
  `_ROADMAP_MARKDOWN_NAME_RE` matches `phase-plan-v7-SEMPHASETAIL` and
  `phase-plan-v5-gagov`, and `indexFile(...)` routes those documents to the
  lightweight Markdown path instead of the full AST/chunk pipeline. That
  means this phase should not assume the answer is a brand-new blanket
  `plans/phase-plan-*.md` exemption. The plan must instead prove where this
  exact mixed-version pair still leaks lexical watchdog budget in the live
  force-full path.
- Operator status already advertises several exact lexical seams in
  ...
  `.claude/commands`, the later v7-only phase-plan pair, the historical
  `WATCH -> p19` pair, exact bounded Python pairs, exact bounded JSON
  seams, and ignored `test_workspace/` fixtures. There is currently no
  dedicated status helper or wording for the exact mixed-version phase-plan
  pair `plans/phase-plan-v7-SEMPHASETAIL.md ->
  plans/phase-plan-v5-gagov.md`.
- Existing tests already freeze adjacent behavior but not this exact pair.
  `tests/test_dispatcher.py` covers bounded Markdown routing plus the exact
  later v7-only and historical phase-plan pairs,
  ...
  evidence artifact to retain the newly exposed mixed-version pair and the
  roadmap steering to downstream phase `SEMMIXEDPHASETAIL`. What is missing
  is the execution-ready contract that binds dispatcher behavior, durable
  trace progression, operator wording, evidence refresh, and possible
  downstream roadmap steering to this exact mixed-version seam.

Practical planning boundary:

- SEMMIXEDPHASETAIL may tighten exact mixed-version phase-plan handling,
  ...
  `plans/phase-plan-v7-SEMPHASETAIL.md ->
  plans/phase-plan-v5-gagov.md` or surfaces a truthful newer blocker.
- SEMMIXEDPHASETAIL must stay narrow and evidence-driven. It must not reopen
  the repaired later v7-only or historical phase-plan seams, introduce a
  broad `plans/phase-plan-*.md` exemption beyond the pre-existing generic
...
- [ ] IF-0-SEMMIXEDPHASETAIL-1 - Mixed-version phase-plan pair advance
      contract: a refreshed repo-local force-full rerun on the
      post-SEMLEGACYPLANS head no longer terminalizes with the durable
      lexical trace centered on
      `plans/phase-plan-v7-SEMPHASETAIL.md ->
      plans/phase-plan-v5-gagov.md`; it either advances durably beyond that
      pair or emits a truthful newer blocker before the 120-second watchdog
      expires.
- [ ] IF-0-SEMMIXEDPHASETAIL-2 - Exact boundary contract:
      any repair introduced by this phase remains limited to the exact
      mixed-version phase-plan pair and the immediate dispatcher/Markdown
      plumbing needed to clear it. The phase must not introduce a new
      blanket `plans/phase-plan-*.md` exemption beyond the pre-existing
      generic lightweight phase-plan naming rule.
- [ ] IF-0-SEMMIXEDPHASETAIL-3 - Lexical discoverability contract:
      ...
      heading discoverability for frontmatter title, section headings, and
      the legacy `.codex/phase-loop/` prose carried by `GAGOV`, instead of
      turning either plan into an indexing blind spot.
- [ ] IF-0-SEMMIXEDPHASETAIL-4 - Durable trace and operator contract:
      `force_full_exit_trace.json` and `uv run mcp-index repository status`
      stay aligned with the repaired outcome for the exact mixed-version
      phase-plan pair and do not regress to stale SEMLEGACYPLANS-only
      wording once the live rerun advances past it.
- [ ] IF-0-SEMMIXEDPHASETAIL-5 - Evidence and downstream steering contract:
      `docs/status/SEMANTIC_DOGFOOD_REBUILD.md` records the SEMLEGACYPLANS
      rerun outcome, the SEMMIXEDPHASETAIL rerun command and timestamps, the
      refreshed trace/status output, and the final authoritative verdict for
      the mixed-version phase-plan pair; if execution reveals a blocker
...
- SL-0 - Exact mixed-version phase-plan fixture freeze; Depends on: SEMLEGACYPLANS; Blocks: SL-1, SL-2, SL-3, SL-4; Parallel-safe: no
- SL-1 - Bounded mixed-version phase-plan markdown execution-path repair; Depends on: SL-0; Blocks: SL-2, SL-3, SL-4; Parallel-safe: no
...
- SL-4 - Downstream roadmap steering reducer; Depends on: SL-0, SL-1, SL-2, SL-3; Blocks: SEMMIXEDPHASETAIL acceptance; Parallel-safe: no

Lane DAG:

```text
SEMLEGACYPLANS
  -> SL-0 -> SL-1 -> SL-2 -> SL-3 -> SL-4 -> SEMMIXEDPHASETAIL acceptance
```

## Lanes

### SL-0 - Exact Mixed-Version Phase-Plan Fixture Freeze

- **Scope**: Freeze the exact
  `plans/phase-plan-v7-SEMPHASETAIL.md ->
  plans/phase-plan-v5-gagov.md` lexical seam in deterministic dispatcher
...
- **Interfaces provided**: executable assertions for
  IF-0-SEMMIXEDPHASETAIL-1,
  IF-0-SEMMIXEDPHASETAIL-2,
  and IF-0-SEMMIXEDPHASETAIL-3 at the dispatcher/plugin boundary
- **Interfaces consumed**: existing
  ...
  and the SEMLEGACYPLANS evidence for the mixed-version phase-plan pair
- **Parallel-safe**: no
- **Tasks**:
  - test: Extend `tests/test_dispatcher.py` with a repo-shaped fixture that
    indexes exact copies or trimmed representatives of
    `plans/phase-plan-v7-SEMPHASETAIL.md` and
    `plans/phase-plan-v5-gagov.md`, proves the lexical walker records the
  ...
  - test: In the same dispatcher coverage, assert that the repaired path
    ...
    through the bounded Markdown shard, including the `GAGOV` title and
    headings that coexist with legacy `.codex/phase-loop/` prose.
  - test: Add a negative guard that an unrelated phase-plan Markdown
    document outside the exact pair still uses the normal generic
    lightweight-name rule or heavy Markdown path as appropriate; the
  ...
  - verify: `uv run pytest tests/test_dispatcher.py -q --no-cov -k "phase_plan or SEMPHASETAIL or gagov or markdown or lexical or bounded"`

### SL-1 - Bounded Mixed-Version Phase-Plan Markdown Execution-Path Repair
...
- **Scope**: Implement the smallest dispatcher or Markdown-path repair needed
  ...
  exact `SEMPHASETAIL -> phase-plan-v5-gagov.md` phase-plan pair.
- **Owned files**: `mcp_server/dispatcher/dispatcher_enhanced.py`, `mcp_server/plugins/markdown_plugin/plugin.py`
- **Interfaces provided**: IF-0-SEMMIXEDPHASETAIL-2 exact boundary contract;
  IF-0-SEMMIXEDPHASETAIL-3 lexical discoverability contract
- **Interfaces consumed**: SL-0 mixed-version phase-plan fixtures; existing
  ...
  `.claude` commands, later v7-only phase plans, and the repaired
  `WATCH -> p19` pair
- **Parallel-safe**: no
- **Tasks**:
  - test: Run the SL-0 dispatcher slice first and confirm what still makes
    the exact mixed-version pair expensive in the current force-full path
  ...
  - impl: Apply the narrowest repair that clears the exact pair. Prefer
    ...
    path; only add exact-path or mixed-version-shape logic if the tests
    prove the generic phase-plan rule is still insufficient for this pair in
    the live walker.
  ...
  - impl: Do not introduce a new blanket `plans/phase-plan-*.md`
    dispatcher bypass, and do not widen into a repo-wide Markdown timeout
    change that masks other later blockers.
  - verify: `uv run pytest tests/test_dispatcher.py -q --no-cov -k "phase_plan or SEMPHASETAIL or gagov or markdown or bounded"`

### SL-2 - Durable Trace Persistence And Operator Boundary Alignment

- **Scope**: Carry the bounded mixed-version phase-plan repair through
  durable trace persistence and operator status output so the live rerun
  truthfully advances beyond the pair without stale SEMLEGACYPLANS-only
  boundary wording.
...
- **Interfaces provided**: IF-0-SEMMIXEDPHASETAIL-1 mixed-version
  phase-plan pair advance contract; IF-0-SEMMIXEDPHASETAIL-4 durable trace
...
- **Interfaces consumed**: SL-0 dispatcher fixtures; SL-1 repaired bounded
  ...
  helpers for validation docs, benchmark docs, `.claude`, later v7-only
  phase plans, historical phase plans, scripts, JSON seams, and ignored
  `test_workspace/` fixtures
- **Parallel-safe**: no
- **Tasks**:
  - test: Extend `tests/test_git_index_manager.py` so force-full durable
    trace persistence freezes the exact mixed-version phase-plan handoff and
    proves later reruns do not regress to stale
    `plans/phase-plan-v6-WATCH.md ->
    plans/phase-plan-v1-p19.md` or SEMLEGACYPLANS wording once execution
    advances past the current pair.
  - test: Extend `tests/test_repository_commands.py` so
    `uv run mcp-index repository status` must advertise the exact
    mixed-version Markdown boundary
    `plans/phase-plan-v7-SEMPHASETAIL.md ->
    plans/phase-plan-v5-gagov.md` when both files exist and the durable
    trace has already advanced into or beyond that pair.
  - impl: Add the smallest boundary-printer and durable-trace alignment
    needed for the exact mixed-version pair beside the existing validation,
    benchmark, `.claude`, script, JSON, later v7-only, and historical
    phase-plan helpers.
  - impl: Preserve the current trace vocabulary fields `Trace status`,
    ...
    them with the repaired mixed-version phase-plan pair outcome.
  - impl: Do not generalize status wording into all phase-plan docs or a
    ...
    exact.
  - verify: `env OPENAI_API_KEY=dummy-local-key uv run pytest tests/test_git_index_manager.py tests/test_repository_commands.py -q --no-cov -k "phase_plan or SEMPHASETAIL or gagov or lexical or interrupted or boundary"`

### SL-3 - Live Rerun Evidence Reducer And Contract Refresh

- **Scope**: Re-run the repaired repo-local force-full sequence and reduce
  the outcome into the durable semantic dogfood evidence artifact.
- **Owned files**: `docs/status/SEMANTIC_DOGFOOD_REBUILD.md`, `tests/docs/test_semdogfood_evidence_contract.py`
- **Interfaces provided**: IF-0-SEMMIXEDPHASETAIL-1 mixed-version
  phase-plan pair advance contract; IF-0-SEMMIXEDPHASETAIL-4 durable trace
  and operator contract; the evidence portion of IF-0-SEMMIXEDPHASETAIL-5
- **Interfaces consumed**: SL-0 mixed-version phase-plan fixtures; SL-1
  repaired dispatcher/Markdown behavior; SL-2 durable trace and status
  wording; the current SEMLEGACYPLANS evidence block; and the live repo-local
...
- **Tasks**:
  - test: Extend `tests/docs/test_semdogfood_evidence_contract.py` so the
    evidence artifact must retain the exact mixed-version phase-plan pair,
    the SEMMIXEDPHASETAIL rerun command, the refreshed trace/status fields,
    the phase plan reference, and the final verdict on whether the active
    blocker moved beyond `plans/phase-plan-v5-gagov.md`.
  - test: In the same docs contract, require the semantic dogfood evidence
    to preserve the existing historical and later v7-only phase-plan seam
    notes while adding the new mixed-version seam and any truthful
    downstream successor.
  - impl: Refresh `docs/status/SEMANTIC_DOGFOOD_REBUILD.md` with the
    SEMLEGACYPLANS observed mixed-version phase-plan blocker, the repaired
    SEMMIXEDPHASETAIL rerun command and timestamps, the resulting durable
...
- **Tasks**:
  ...
  - impl: If the active blocker is still the exact mixed-version phase-plan
...
- Whole-phase checks:
  - `env OPENAI_API_KEY=dummy-local-key uv run pytest tests/test_dispatcher.py tests/test_git_index_manager.py tests/test_repository_commands.py tests/docs/test_semdogfood_evidence_contract.py -q --no-cov -k "phase_plan or SEMPHASETAIL or gagov or markdown or lexical or bounded or interrupted or boundary"`
  - `timeout 120s env OPENAI_API_KEY=dummy-local-key uv run mcp-index repository sync --force-full`
...
- [ ] A refreshed repo-local force-full rerun on the post-SEMLEGACYPLANS
      head either advances durably beyond
      `plans/phase-plan-v7-SEMPHASETAIL.md ->
      plans/phase-plan-v5-gagov.md` or emits a truthful newer blocker
...
- [ ] Both exact phase-plan files remain lexically discoverable with durable
      ...
      including the mixed-version `GAGOV` structure that still references
      legacy `.codex/phase-loop/` runner state in prose.
- [ ] `uv run mcp-index repository status` and
      `force_full_exit_trace.json` stay aligned with the repaired
      mixed-version phase-plan boundary outcome and do not regress to stale
      SEMLEGACYPLANS wording.
- [ ] `docs/status/SEMANTIC_DOGFOOD_REBUILD.md` records the SEMLEGACYPLANS
      rerun outcome and the final live verdict for the mixed-version
      phase-plan pair.
...
automation:
  ...
  next_command: codex-execute-phase /home/viperjuice/code/Code-Index-MCP/plans/phase-plan-v7-SEMMIXEDPHASETAIL.md
  ...
  artifact: /home/viperjuice/code/Code-Index-MCP/plans/phase-plan-v7-SEMMIXEDPHASETAIL.md
