    ...
    try:
        ...
        if potential_risks:
            ...
            for i, risk in enumerate(potential_risks[:5]):
                print(f"  {i+1}. Line {risk.get('line', 0)}: {risk.get('description', 'Unknown risk')}")
    ...
    try:
        ...
        if anti_patterns:
            ...
            for i, pattern in enumerate(anti_patterns):
                print(f"  {i+1}. Line {pattern.get('line', 0)}: {pattern.get('description', 'Unknown')}")
        
        # Get recommendations
        recommendations = plugin.coroutines_analyzer.get_coroutine_recommendations(coroutines_result)
        if recommendations:
    ...
    try:
        ...
        if jvm_annotations:
            ...
            for i, annotation in enumerate(jvm_annotations):
                print(f"  {i+1}. {annotation.get('name', 'Unknown')} at line {annotation.get('line', 0)}")
        ...
        if potential_issues:
            ...
            for i, issue in enumerate(potential_issues[:3]):
                print(f"  {i+1}. Line {issue.get('line', 0)}: {issue.get('description', 'Unknown')}")
