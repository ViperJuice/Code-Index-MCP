...
class MCPTestOrchestrator:
    ...
    async def test_language_plugin(self, language: str, repo_config: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            test_results["performance"] = {
                ...
                "indexing_speed_files_per_sec": round(indexed_files / (end_time - start_time), 2) if end_time > start_time else 0,
    ...
    async def run_comprehensive_tests(self) -> Dict[str, Any]:
        ...
        logger.info(f"Downloaded {successful_downloads}/{len(SMALL_TEST_REPOSITORIES)} repositories")
    ...
    def _calculate_overall_stats(self):
        ...
        successful_plugins = sum(1 for p in plugins_tested.values() if p.get("plugin_loaded", False))
        ...
        self.test_results["overall_stats"] = {
            ...
            "plugin_success_rate": round(successful_plugins / total_plugins * 100, 1) if total_plugins > 0 else 0,
            "total_files_indexed": total_files,
            "total_symbols_extracted": total_symbols,
            "symbol_lookup_success_rate": round(successful_lookups / total_lookups * 100, 1) if total_lookups > 0 else 0,
            "search_success_rate": round(successful_searches / total_searches * 100, 1) if total_searches > 0 else 0
        }
    ...
    def _save_results(self):
        ...
        summary = {
            ...
            "mcp_integration_success": self.test_results.get("mcp_integration", {}).get("mcp_server_startup", False)
        }
    ...
    def print_results_summary(self):
        ...
        if self.test_results.get("errors"):
            ...
            for error in self.test_results["errors"][:5]:  # Show first 5 errors
                print(f"   - {error.get('stage', 'unknown')}: {error.get('error', 'Unknown error')[:60]}...")
