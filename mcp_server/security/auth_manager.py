...
from .models import (
    ...
    SessionInfo, DEFAULT_ROLE_PERMISSIONS, DEFAULT_ACCESS_RULES, AccessLevel
)
...
class AuthManager(IAuthenticator, IAuthorizer):
    ...
    async def authenticate_user(self, credentials: AuthCredentials) -> Optional[User]:
        ...
        if not user:
            ...
            await self._log_security_event("authentication_failed", username=username, details={"reason": "user_not_found"})
            raise AuthenticationError("Invalid credentials")
        
        if not user.is_active:
            await self._log_security_event("authentication_failed", username=username, details={"reason": "user_inactive"})
            raise AuthenticationError("Account is inactive")
        
        if not self.password_manager.verify_password(credentials.password, user.hashed_password):
            await self._increment_failed_attempts(username)
            await self._log_security_event("authentication_failed", username=username, details={"reason": "invalid_password"})
            raise AuthenticationError("Invalid credentials")
    ...
    async def _increment_failed_attempts(self, username: str):
        ...
        if self.failed_attempts[username] >= self.config.max_login_attempts:
            ...
            await self._log_security_event("account_locked", username=username, 
    ...
    async def create_access_token(self, user: User) -> str:
        ...
        await self._log_security_event("access_token_created", user_id=user.id, username=user.username)
        return token
    ...
    async def create_refresh_token(self, user: User) -> str:
        ...
        token = jwt.encode(payload, self.config.jwt_secret_key, algorithm=self.config.jwt_algorithm)
        await self._log_security_event("refresh_token_created", user_id=user.id, username=user.username)
        return token
    ...
    async def refresh_access_token(self, refresh_token: str) -> Optional[str]:
        ...
        try:
            ...
            await self._log_security_event("token_refreshed", user_id=user.id, username=user.username)
    ...
    async def revoke_refresh_token(self, refresh_token: str) -> bool:
        ...
        try:
            ...
            if token_id and token_id in self.refresh_tokens:
                ...
                await self._log_security_event("refresh_token_revoked", details={"token_id": token_id})
                return True
    ...
    async def check_role(self, user: User, required_role: UserRole) -> bool:
        """Check if user has required role or higher."""
        role_hierarchy = [UserRole.GUEST, UserRole.READONLY, UserRole.USER, UserRole.ADMIN]
    ...
    async def add_access_rule(self, rule: AccessRule):
        ...
        await self._log_security_event("access_rule_added", details={"rule_id": rule.id, "pattern": rule.path_pattern})
    ...
    async def remove_access_rule(self, rule_id: str) -> bool:
        ...
        for i, rule in enumerate(self.access_rules):
            if rule.id == rule_id:
                ...
                await self._log_security_event("access_rule_removed", details={"rule_id": rule_id, "pattern": removed_rule.path_pattern})
                return True
        return False
