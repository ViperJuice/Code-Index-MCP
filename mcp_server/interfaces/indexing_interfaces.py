...
from .plugin_interfaces import SymbolDefinition, SymbolReference, SearchResult, IndexedFile
...
class IIndexEngine(IObservable):
    ...
    @abstractmethod
    async def index_directory(self, directory: str, recursive: bool = True) -> Result[List[IndexedFile]]:
...
class IParseCoordinator(ABC):
    ...
    @abstractmethod
    async def parse_file(self, file_path: str, content: Optional[str] = None) -> Result[List[SymbolDefinition]]:
...
class ISearchEngine(ABC):
    ...
    @abstractmethod
    async def search_symbols(self, query: str, options: Dict[str, Any]) -> Result[List[SymbolDefinition]]:
        """Search for symbols"""
        pass
    
    @abstractmethod
    async def search_references(self, symbol: str, options: Dict[str, Any]) -> Result[List[SymbolReference]]:
        """Search for symbol references"""
        pass
    
    @abstractmethod
    async def search_content(self, query: str, options: Dict[str, Any]) -> Result[List[SearchResult]]:
        """Search file content"""
        pass
    ...
    @abstractmethod
    async def search_semantic(self, query: str, options: Dict[str, Any]) -> Result[List[SemanticMatch]]:
...
class ISemanticIndexer(ABC):
    ...
    @abstractmethod
    async def generate_embeddings(self, content: str, metadata: Dict[str, Any] = None) -> Result[List[float]]:
        """Generate embeddings for content"""
        pass
    
    @abstractmethod
    async def index_embeddings(self, embeddings: List[float], content: str, file_path: str) -> Result[None]:
        """Index embeddings"""
        pass
    
    @abstractmethod
    async def search_similar(self, query_embedding: List[float], top_k: int = 10) -> Result[List[SemanticMatch]]:
...
class IIndexManager(ABC):
    ...
    @abstractmethod
    async def create_index(self, index_name: str, index_type: str, config: Dict[str, Any]) -> Result[None]:
        """Create a new index"""
        pass
...
class IIndexStore(IAsyncRepository[IndexEntry]):
    ...
    @abstractmethod
    async def search_symbols(self, query: str, filters: Dict[str, Any]) -> Result[List[SymbolDefinition]]:
