...
class IAuthorizer(ABC):
    ...
    @abstractmethod
    def is_authorized(self, user: User, resource: str, action: str, context: Dict[str, Any] = None) -> bool:
...
class IAccessController(ABC):
    ...
    @abstractmethod
    async def grant_access(self, user_id: str, resource: str, permissions: List[str]) -> Result[None]:
        """Grant access to a user"""
        pass
    
    @abstractmethod
    async def revoke_access(self, user_id: str, resource: str, permissions: List[str]) -> Result[None]:
...
class IPolicyEngine(ABC):
    ...
    @abstractmethod
    async def list_applicable_policies(self, context: Dict[str, Any]) -> Result[List[SecurityPolicy]]:
...
class ISecurityAuditor(ABC):
    ...
    @abstractmethod
    async def log_authentication_attempt(self, username: str, success: bool, ip_address: str) -> None:
        """Log authentication attempt"""
        pass
    
    @abstractmethod
    async def log_authorization_check(self, user_id: str, resource: str, action: str, allowed: bool) -> None:
...
class ISecretManager(ABC):
    ...
    @abstractmethod
    async def store_secret(self, key: str, value: str, metadata: Dict[str, Any] = None) -> Result[None]:
        """Store a secret"""
        pass
...
class ISecurityMiddleware(ABC):
    ...
    @abstractmethod
    async def authorize_request(self, context: ISecurityContext, resource: str, action: str) -> Result[bool]:
        """Authorize a request"""
        pass
