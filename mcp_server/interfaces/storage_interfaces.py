...
class IQueryEngine(ABC):
    ...
    async def select(self, table: str, columns: List[str] = None, where: Dict[str, Any] = None, 
                    order_by: List[str] = None, limit: int = None, offset: int = None) -> Result[QueryResult]:
    ...
    @abstractmethod
    async def upsert(self, table: str, data: Dict[str, Any], conflict_columns: List[str]) -> Result[int]:
...
class ISchemaManager(ABC):
    ...
    @abstractmethod
    async def create_index(self, table_name: str, index_name: str, columns: List[str], unique: bool = False) -> Result[None]:
        """Create an index"""
        pass
...
class IFTSEngine(ABC):
    ...
    @abstractmethod
    async def create_fts_table(self, table_name: str, columns: List[str], options: Dict[str, Any] = None) -> Result[None]:
        """Create a FTS table"""
        pass
    
    @abstractmethod
    async def index_content(self, table_name: str, document_id: str, content: Dict[str, str]) -> Result[None]:
        """Index content for FTS"""
        pass
    
    @abstractmethod
    async def search_fts(self, table_name: str, query: str, options: Dict[str, Any] = None) -> Result[List[Dict[str, Any]]]:
        """Search using FTS"""
        pass
    
    @abstractmethod
    async def update_fts_content(self, table_name: str, document_id: str, content: Dict[str, str]) -> Result[None]:
        """Update FTS content"""
        pass
...
class ITextSearcher(ABC):
    ...
    @abstractmethod
    async def search_wildcard(self, pattern: str, field: str = None) -> Result[List[Dict[str, Any]]]:
        """Wildcard search"""
        pass
    
    @abstractmethod
    async def get_search_suggestions(self, partial_query: str, limit: int = 10) -> Result[List[str]]:
...
class ISymbolRepository(ABC, IAsyncRepository[SymbolDefinition]):
    ...
    @abstractmethod
    async def search_symbols(self, query: str, options: Dict[str, Any] = None) -> List[SymbolDefinition]:
        """Search symbols"""
        pass
...
class IMigrationRunner(ABC):
    ...
    @abstractmethod
    async def create_migration(self, description: str, up_script: str, down_script: str) -> Result[str]:
...
class IBackupManager(ABC):
    ...
    @abstractmethod
    async def create_backup(self, backup_type: str = "full", options: Dict[str, Any] = None) -> Result[BackupInfo]:
        """Create a backup"""
        pass
