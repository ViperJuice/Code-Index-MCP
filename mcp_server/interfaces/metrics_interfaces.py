...
class IMetricsCollector(IMetrics):
    ...
    @abstractmethod
    def get_metric_history(self, metric_name: str, start_time: datetime, end_time: datetime) -> List[MetricSample]:
        """Get metric history"""
        pass
...
class IMetricsExporter(ABC):
    ...
    @abstractmethod
    async def export_metrics(self, metrics: List[MetricSample], format: str = "prometheus") -> Result[str]:
...
class IHealthMonitor(ABC):
    ...
    @abstractmethod
    async def register_health_check(self, name: str, check_func: Callable[[], HealthCheckResult]) -> None:
...
class IPrometheusExporter(IMetricsExporter):
    ...
    @abstractmethod
    def register_prometheus_metric(self, metric_name: str, metric_type: str, description: str, labels: List[str] = None) -> None:
        """Register a Prometheus metric"""
        pass
    
    @abstractmethod
    def update_prometheus_metric(self, metric_name: str, value: float, labels: Dict[str, str] = None) -> None:
...
class IStructuredLogger(ABC):
    ...
    @abstractmethod
    def log_with_metrics(self, level: str, message: str, metrics: Dict[str, float], context: Dict[str, Any] = None) -> None:
    ...
    @abstractmethod
    def log_request(self, method: str, path: str, status_code: int, duration: float, correlation_id: str) -> None:
...
class IBusinessMetrics(ABC):
    ...
    @abstractmethod
    def track_plugin_performance(self, plugin_name: str, operation: str, duration: float, success: bool) -> None:
