...
class IDispatcher(ABC):
    ...
    @abstractmethod
    async def dispatch_to_plugin(self, plugin_name: str, request: DispatchRequest) -> Result[DispatchResult]:
...
class IResultAggregator(ABC):
    ...
    @abstractmethod
    def merge_plugin_results(self, operation: str, results: List[DispatchResult]) -> AggregatedResult:
...
class IResultFilter(ABC):
    ...
    @abstractmethod
    def filter_by_file_type(self, results: List[SearchResult], file_types: List[str]) -> List[SearchResult]:
        """Filter results by file type"""
        pass
    
    @abstractmethod
    def apply_custom_filter(self, results: List[SearchResult], filter_func: Callable[[SearchResult], bool]) -> List[SearchResult]:
...
class ILoadBalancer(ABC):
    ...
    @abstractmethod
    def select_plugin(self, available_plugins: List[str], request: DispatchRequest) -> Optional[str]:
...
class IDispatchMonitor(ABC):
    ...
    @abstractmethod
    def record_plugin_execution(self, plugin_name: str, operation: str, execution_time: float, success: bool) -> None:
...
class IDispatchTracer(ABC):
    ...
    @abstractmethod
    def add_span(self, trace_id: str, plugin_name: str, operation: str, start_time: float, end_time: float) -> None:
