...
class BaseDocumentPlugin(SpecializedPluginBase):
    ...
    def _chunk_text(self, text: str, start_offset: int = 0, 
                    chunk_index_start: int = 0) -> List[DocumentChunk]:
        ...
        for file_path, chunks in self._chunk_cache.items():
            for chunk in chunks:
                if symbol.lower() in chunk.content.lower():
                    ...
                    lines_before = chunk.content[:chunk.content.lower().index(symbol.lower())].count('\n')
    ...
    def _extract_text_around_position(self, content: str, position: int, 
                                    radius: int = 100) -> str:
