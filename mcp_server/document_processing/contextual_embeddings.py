...
class ContextualEmbeddingService:
    ...
    def detect_document_category(self, chunk: DocumentChunk, document_path: str) -> DocumentCategory:
    ...
    async def generate_context_for_chunk(
        ...
        async with self.semaphore:
            try:
                ...
                return f"Content from {chunk.metadata.document_path}: {chunk.content[:100]}...", False
    ...
    async def _process_chunk_with_progress(
        ...
        if progress_callback:
            progress_callback(
                self.current_metrics.processed_chunks,
                self.current_metrics.total_chunks
            )
