...
class PreFlightValidator:
    ...
    async def validate_all(self) -> Dict[str, Any]:
        ...
        results["overall_status"] = "passed" if all_passed else ("warning" if critical_passed else "failed")
    ...
    async def _validate_mcp_tools(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result["details"]["inheritance_enabled"] = os.environ.get("MCP_INHERIT_CONFIG") == "true"
    ...
    def _generate_recommendations(self, checks: Dict[str, Any]) -> List[str]:
        ...
        if mcp_check.get("status") == "failed":
            if mcp_check.get("details", {}).get("is_sub_agent"):
                recommendations.append("Enable MCP tool inheritance: export MCP_INHERIT_CONFIG=true")
            else:
        ...
        if index_check.get("status") == "failed":
            if not index_check.get("details", {}).get("index_enabled"):
                recommendations.append("Enable indexing: Create .mcp-index.json with {\"enabled\": true}")
            else:
                recommendations.append("Create index: Run 'mcp-index index' or 'claude-index create'")
...
class MCPHealthCheck:
    ...
    async def check_mcp_available() -> Tuple[bool, str]:
        ...
        try:
            ...
            mcp_servers = sum(1 for k in os.environ if k.startswith("MCP_SERVER_") and k.endswith("_COMMAND"))
            if mcp_servers > 0:
