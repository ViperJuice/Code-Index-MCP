    ...
    try:
        ...
        for repo_id, repo_info in repos.items():
            ...
            if verbose:
                ...
                click.echo(f"  Indexed commit: {repo_info.last_indexed_commit[:8] if repo_info.last_indexed_commit else 'Never'}")
                click.echo(f"  Last indexed: {repo_info.last_indexed or 'Never'}")
                click.echo(f"  Sync: {sync_status}")
                click.echo(f"  Artifacts: {'enabled' if repo_info.artifact_enabled else 'disabled'}")
        ...
        if sync_all:
            ...
            for rid, result in results.items():
                ...
                if repo_info:
                    if result.action == "up_to_date":
                        click.echo(click.style(f"✓ {repo_info.name}: Already up to date", fg='green'))
                        ...
                        click.echo(click.style(f"✓ {repo_info.name}: Downloaded from artifact", fg='green'))
                    else:
            ...
            if not repo_id:
                ...
                if repo_info:
                    ...
                    click.echo(click.style("✗ Current directory is not a registered repository", fg='red'), err=True)
...
def status(repo_id: Optional[str]):
    ...
    try:
        ...
        click.echo(f"  Current commit: {status['current_commit'][:8] if status['current_commit'] else 'None'}")
        click.echo(f"  Indexed commit: {status['last_indexed_commit'][:8] if status['last_indexed_commit'] else 'Never'}")
...
@click.option('--register/--no-register', default=True, help='Automatically register found repositories')
        ...
        for repo_path in discovered:
            ...
            if register:
                try:
                    ...
                    click.echo(click.style(f"  ✓ {repo_name} (registered as {repo_id[:8]}...)", fg='green'))
                    registered_count += 1
                except Exception as e:
...
def watch(watch_all: bool, daemon: bool):
    ...
    try:
        ...
        if not watch_all:
            click.echo(click.style("✗ Please specify --all to watch all repositories", fg='red'), err=True)
