...
class FuzzyIndexer:
    ...
    def add_file(self, path: str, content: str) -> None:
        ...
        # If using SQLite backend, also store in FTS5 table
        if self.sqlite_store:
            try:
                ...
                logger.error(f"Failed to update FTS5 index for {path}: {e}")
    ...
    def _search_fts5(self, query: str, limit: int) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"FTS5 search failed, falling back to memory: {e}")
            return self._search_memory(query, limit)
    ...
    def clear(self) -> None:
        ...
        # Also clear FTS5 data if using SQLite
        if self.sqlite_store:
            try:
                with self.sqlite_store._get_connection() as conn:
                    conn.execute("DELETE FROM fts_code")
            except Exception as e:
                logger.error(f"Failed to clear FTS5 index: {e}")
    
    # ------------------------------------------------------------------
    def add_symbol(self, symbol_name: str, file_path: str, line_number: int, 
