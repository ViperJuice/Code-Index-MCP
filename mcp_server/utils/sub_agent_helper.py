...
class SubAgentMCPHelper:
    ...
    def _load_inherited_config(self) -> Dict[str, Any]:
        ...
        logger.info(f"Loaded config for {len(config['servers'])} servers and {len(config['tools'])} tools")
        return config
...
def inherit_mcp_tools() -> Optional[SubAgentMCPHelper]:
    ...
    logger.info(f"MCP tool inheritance complete: {available_count}/{len(availability)} tools available")
