...
class DirectSearcher:
    ...
    def search_pattern(self, pattern: str, path: str = ".", use_ripgrep: bool = True) -> Dict[str, Any]:
        """
    ...
    def search_string(self, string: str, path: str = ".", use_ripgrep: bool = True) -> Dict[str, Any]:
        """
    ...
    def _execute_command(self, cmd: List[str], tool_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            elapsed_time = time.time() - start_time
            return self._error_result(f"Command timed out after 30 seconds", tool_name, cmd, elapsed_time)
    ...
    def _error_result(self, error_msg: str, tool_name: str = None, cmd: List[str] = None, elapsed_time: float = 0.0) -> Dict[str, Any]:
    ...
    def compare_tools(self, pattern: str, path: str = ".", is_literal: bool = False) -> Dict[str, Any]:
        """
        ...
        if self.has_ripgrep:
            results["ripgrep"] = self.measure_performance(search_method, pattern, path, use_ripgrep=True)
        
        # Run grep if available
        if self.has_grep:
            results["grep"] = self.measure_performance(search_method, pattern, path, use_ripgrep=False)
        ...
        if len(results) == 2:
            ...
            results["comparison"] = {
                ...
                "matches_equal": results["ripgrep"]["match_count"] == results["grep"]["match_count"]
            }
