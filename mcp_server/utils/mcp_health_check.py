...
class MCPDiagnostics:
    ...
    async def _diagnose_tool_availability(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Determine issues
            if not tool_checks["config_propagation"] and os.environ.get("MCP_SUB_AGENT_ACCESS") == "true":
                ...
                try:
                    ...
                    if "code-index-mcp" in servers:
                        ...
                        if not server_config.get("sub_agent_access"):
                            result["issues"].append("Server config missing 'sub_agent_access: true'")
                    else:
    ...
    async def _diagnose_index_access(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not index_info["enabled"]:
                result["issues"].append("MCP indexing not enabled (.mcp-index.json missing or disabled)")
...
async def quick_mcp_check() -> bool:
    ...
    try:
        ...
        has_servers = any(k.startswith("MCP_SERVER_") and k.endswith("_COMMAND") for k in os.environ)
