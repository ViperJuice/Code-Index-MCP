...
from .cache_manager import ICacheManager, CacheManager
...
class QueryResultCache:
    ...
    def _extract_file_dependencies(self, query_type: QueryType, 
                                  result: Any, **params) -> Set[str]:
    ...
    def _track_query_popularity(self, query_key: str):
        ...
        if (self._query_stats[query_key] >= self.config.popular_queries_threshold and
            query_key not in self._popular_queries):
    ...
    async def warm_popular_queries(self, query_factory: Callable[[str, QueryType], Awaitable[Any]]) -> int:
    ...
    async def get_cache_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "query_frequency": dict(sorted(
                    self._query_stats.items(), 
                    key=lambda x: x[1], 
                    reverse=True
                )[:10])  # Top 10 most frequent queries
            }
