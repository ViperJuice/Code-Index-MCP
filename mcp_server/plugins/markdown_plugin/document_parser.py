...
class MarkdownParser:
        ...
        if start + 1 < len(lines):
            ...
            if re.match(r'^=+\s*$', next_line):
                node = {
                    ...
                    "position": {"start": {"line": start + 1}, "end": {"line": start + 2}}
                }
                return node, start + 2
    ...
    def _parse_code_block(self, lines: List[str], start: int) -> Optional[Tuple[Dict[str, Any], int]]:
        """Parse a code block."""
        line = lines[start]
            ...
            while i < len(lines):
                if lines[i].strip() == '```':
                    node = {
                        ...
                        "lang": lang,
                        "value": '\n'.join(code_lines),
                        "position": {"start": {"line": start + 1}, "end": {"line": i + 1}}
                    }
                    return node, i + 1
    ...
    def _parse_blockquote(self, lines: List[str], start: int) -> Optional[Tuple[Dict[str, Any], int]]:
        """Parse a blockquote."""
        line = lines[start]
        ...
        if line.startswith('>'):
            ...
            while i < len(lines) and (lines[i].startswith('>') or (quote_lines and not lines[i].strip())):
    ...
    def _parse_table(self, lines: List[str], start: int) -> Optional[Tuple[Dict[str, Any], int]]:
        ...
        if start + 1 < len(lines):
            ...
            delimiter_match = re.match(r'^\|?[\s:]*-+[\s:]*(\|[\s:]*-+[\s:]*)*\|?$', lines[start + 1])
    ...
    def _parse_horizontal_rule(self, lines: List[str], start: int) -> Optional[Tuple[Dict[str, Any], int]]:
    ...
    def _parse_paragraph(self, lines: List[str], start: int) -> Optional[Tuple[Dict[str, Any], int]]:
        """Parse a paragraph."""
        paragraph_lines = []
        i = start
    ...
    def _parse_inline(self, text: str) -> List[Dict[str, Any]]:
        ...
        for match_type, match in matches:
            ...
            if match.start() > last_end:
                children.append({
                    "type": "text",
                    "value": text[last_end:match.start()]
                })
