...
class SwiftProtocolChecker:
    ...
    def validate_conformance(self, type_name: str, protocol_name: str, content: str) -> Dict[str, Any]:
    ...
    def find_protocol_extensions(self, content: str, file_path: str = "") -> List[Dict[str, Any]]:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if extension_match:
                ...
                if (protocol_name in self.protocols or 
                    protocol_name in self.system_protocols):
    ...
    def _find_direct_conformances(self, line: str, file_path: str, line_num: int) -> List[ProtocolConformance]:
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                for conf_str in conformances_str:
                    ...
                    if (conf_str and 
                        (conf_str in self.system_protocols or 
                         conf_str in self.protocols or
                         self._looks_like_protocol(conf_str))):
    ...
    def _find_extension_conformances(self, line: str, file_path: str, line_num: int) -> List[ProtocolConformance]:
        ...
        if extension_match:
            ...
            for conf_str in conformances_str:
                if conf_str:
                    conformance_type = (ConformanceType.CONDITIONAL if where_conditions 
    ...
    def _find_property_wrapper_definition(self, line: str, file_path: str, line_num: int) -> Optional[PropertyWrapper]:
        """Find property wrapper definitions."""
        # Look for @propertyWrapper attribute followed by struct/class
        wrapper_match = re.search(r'@propertyWrapper\s*(?:\n\s*)?(?:struct|class)\s+(\w+)', line, re.MULTILINE)
        if wrapper_match:
    ...
    def _find_property_wrapper_usage(self, line: str, file_path: str, line_num: int) -> List[PropertyWrapper]:
        """Find property wrapper usage."""
        wrappers = []
        ...
        if wrapper_usage_match:
            ...
            if (wrapper_name in self.property_wrappers or
                self._looks_like_property_wrapper(wrapper_name)):
    ...
    def _find_result_builder_definition(self, line: str, file_path: str, line_num: int) -> Optional[ResultBuilder]:
    ...
    def _find_result_builder_usage(self, line: str, file_path: str, line_num: int) -> List[str]:
        ...
        if builder_usage_match:
            ...
            if (builder_name in self.result_builders or
                self._looks_like_result_builder(builder_name)):
                builders.append(builder_name)
        
        return builders
    
    def _parse_protocol_body(self, lines: List[str], start_line: int, file_path: str) -> Tuple[List[str], List[str], List[str]]:
    ...
    def _find_implemented_methods(self, type_name: str, content: str) -> List[str]:
        ...
        for line in lines:
            ...
            if re.match(rf'(?:class|struct|enum|actor)\s+{re.escape(type_name)}\b', line_stripped):
                in_type = True
                brace_count = 0
                continue
            ...
            if in_type:
                ...
                if ('func ' in line_stripped or 'var ' in line_stripped or 'subscript' in line_stripped):
    ...
    def _is_protocol_requirement(self, line: str) -> bool:
        ...
        return (('func ' in line or 'var ' in line or 'subscript' in line) and
                '{' not in line and
                not line.endswith('}'))
    ...
    def _looks_like_property_wrapper(self, name: str) -> bool:
        ...
        wrapper_patterns = ['State', 'Binding', 'Published', 'ObservedObject', 'EnvironmentObject']
