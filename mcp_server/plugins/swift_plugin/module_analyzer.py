...
class SwiftModuleAnalyzer:
    ...
    def _parse_import_statement(self, line: str, line_number: int) -> Optional[ImportInfo]:
        ...
        patterns = [
            ...
            (r'^import\s+(class|struct|protocol|enum|func|var|typealias)\s+([\w.]+)$', 'specific'),
    ...
    def validate_imports(self, content: str, available_modules: Set[str]) -> List[Dict[str, Any]]:
        ...
        for import_info in imports:
            ...
            if (module_name not in available_modules and 
                module_name not in self.system_frameworks and
    ...
    def _init_system_frameworks(self) -> Dict[str, List[str]]:
        ...
        return {
            'Foundation': [
                ...
                'NSError', 'NSNotificationCenter', 'NSUserDefaults',
                'NSBundle', 'NSFileManager', 'NSTimer', 'NSThread',
                'NSURL', 'NSURLSession', 'NSJSONSerialization',
                'String', 'Array', 'Dictionary', 'Data', 'Date',
                'NotificationCenter', 'UserDefaults', 'Bundle', 'FileManager',
                'Timer', 'Thread', 'URL', 'URLSession', 'JSONSerialization'
            ],
            'UIKit': [
                'UIView', 'UIViewController', 'UILabel', 'UIButton',
                'UIImageView', 'UITableView', 'UICollectionView',
                'UINavigationController', 'UITabBarController',
                'UIApplication', 'UIWindow', 'UIScreen', 'UIColor',
                'UIFont', 'UIImage', 'UIStoryboard', 'UIAlertController',
                'UITextField', 'UITextView', 'UIScrollView', 'UIStackView'
            ],
            'SwiftUI': [
                'View', 'Text', 'VStack', 'HStack', 'ZStack', 'Button',
                'Image', 'List', 'NavigationView', 'TabView', 'Form',
                'Section', 'Toggle', 'Slider', 'Stepper', 'Picker',
                'State', 'Binding', 'ObservedObject', 'StateObject',
                'EnvironmentObject', 'Environment', 'Published'
            ],
            'Combine': [
                'Publisher', 'Subscriber', 'Subject', 'PassthroughSubject',
                'CurrentValueSubject', 'AnyCancellable', 'Published',
                'ObservableObject', 'Just', 'Future', 'Empty', 'Fail'
            ],
            'CoreData': [
                'NSManagedObject', 'NSManagedObjectContext', 'NSPersistentContainer',
                'NSFetchRequest', 'NSEntityDescription', 'NSPredicate',
                'NSSortDescriptor', 'NSManagedObjectModel', 'NSPersistentStore'
            ],
            'CoreGraphics': [
                'CGFloat', 'CGPoint', 'CGSize', 'CGRect', 'CGColor',
                'CGImage', 'CGContext', 'CGPath', 'CGAffineTransform'
            ],
            'QuartzCore': [
                'CALayer', 'CAAnimation', 'CABasicAnimation', 'CAKeyframeAnimation',
                'CATransition', 'CAShapeLayer', 'CAGradientLayer'
            ],
            'AVFoundation': [
                'AVPlayer', 'AVPlayerItem', 'AVAsset', 'AVAudioPlayer',
                'AVAudioSession', 'AVCaptureSession', 'AVCaptureDevice'
            ],
            'WebKit': [
            ...
            'MapKit': [
                'MKMapView', 'MKAnnotation', 'MKPointAnnotation',
                'MKCoordinateRegion', 'CLLocationCoordinate2D'
            ],
            'CoreLocation': [
                'CLLocationManager', 'CLLocation', 'CLLocationCoordinate2D',
                'CLGeocoder', 'CLPlacemark', 'CLLocationManagerDelegate'
            ],
            'UserNotifications': [
            ...
            'CloudKit': [
                'CKContainer', 'CKDatabase', 'CKRecord', 'CKRecordID',
