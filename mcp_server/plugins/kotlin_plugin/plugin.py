...
from ..specialized_plugin_base import SpecializedPluginBase, IImportResolver, ITypeAnalyzer, IBuildSystemIntegration, ICrossFileAnalyzer
...
class KotlinPlugin(SpecializedPluginBase):
    ...
    def extract_symbols(self, content: str, file_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            symbols.extend(self._create_specialized_symbols(
                null_safety_info, coroutines_info, java_interop_info, gradle_info, file_path
            ))
    ...
    def _extract_basic_symbols(self, tree: tree_sitter.Tree, content: str, file_path: str) -> List[Dict[str, Any]]:
    ...
    def _create_symbol_from_node(self, node: tree_sitter.Node, capture_name: str, 
                                lines: List[str], file_path: str, query_type: str) -> Optional[Dict[str, Any]]:
    ...
    def _extract_node_metadata(self, node: tree_sitter.Node, lines: List[str]) -> Dict[str, Any]:
        ...
        if node.type in ['function_declaration', 'primary_constructor', 'secondary_constructor']:
        ...
        for child in node.children:
            if child.type in ['function_value_parameters', 'class_parameters']:
                for param_child in child.children:
                    if param_child.type in ['function_value_parameter', 'class_parameter']:
                        param_info = {
                            'name': '',
                            'type': '',
                            'modifiers': []
    ...
    def _create_specialized_symbols(self, null_safety_info: Dict, coroutines_info: Dict,
                                  java_interop_info: Dict, gradle_info: Dict, file_path: str) -> List[Dict[str, Any]]:
    ...
    def _search_with_pattern(self, pattern: str, file_path: Optional[str] = None) -> List[Dict[str, Any]]:
