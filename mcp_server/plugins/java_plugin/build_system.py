...
from mcp_server.plugins.specialized_plugin_base import IBuildSystemIntegration, BuildDependency
...
class JavaBuildSystemIntegration(IBuildSystemIntegration):
    ...
    def _parse_maven_pom(self, pom_path: Path) -> List[BuildDependency]:
        ...
        try:
            ...
            group_id = self._find_text(root, './/m:groupId', ns) or self._find_text(root, './/m:parent/m:groupId', ns)
            artifact_id = self._find_text(root, './/m:artifactId', ns)
            version = self._find_text(root, './/m:version', ns) or self._find_text(root, './/m:parent/m:version', ns)
            ...
            if deps_elem is not None:
                for dep in deps_elem.findall('m:dependency', ns):
                    ...
                    if dep_group and dep_artifact:
                        ...
                        if dep_version and dep_version.startswith('${') and dep_version.endswith('}'):
    ...
    def get_project_structure(self) -> Dict[str, Any]:
        ...
        structure['source_dirs'] = [d for d in structure['source_dirs'] 
                                   if (self.project_root / d).exists()]
        structure['test_dirs'] = [d for d in structure['test_dirs'] 
                                 if (self.project_root / d).exists()]
        structure['resource_dirs'] = [d for d in structure['resource_dirs'] 
                                     if (self.project_root / d).exists()]
