...
class JsPluginSemantic(PluginWithSemanticSearch):
    ...
    def _extract_symbols(self, node: Node, content: str, symbols: List[Dict], 
                        ...
                        file_id: Optional[int] = None, scope_path: List[str] = None,
        ...
        if node.type in ['function_declaration', 'function']:
            ...
            if name_node:
                ...
                if self._sqlite_store and file_id:
                    ...
                    if path:
                        self._indexer.add_symbol(name, str(path), symbol_info['line'],
        ...
        # Class declarations
            ...
            if name_node:
                ...
                if body_node:
                    self._extract_symbols(body_node, content, symbols, imports, exports, 
                                        file_id, scope_path + [name], path)
        ...
        for child in node.children:
            self._extract_symbols(child, content, symbols, imports, exports, file_id, scope_path, path)
            ...
            for path in Path(".").rglob(f"*{ext}"):
                try:
                    ...
                    if symbol in content:
                        lines = content.split('\n')
                        for i, line in enumerate(lines):
                            if symbol in line and ('function' in line or 'class' in line or 'const' in line):
                                return SymbolDef(
                                    symbol=symbol,
                                    kind='function' if 'function' in line else 'class' if 'class' in line else 'variable',
                                    language='javascript',
                                    signature=line.strip(),
                                    doc=None,
                                    defined_in=str(path),
