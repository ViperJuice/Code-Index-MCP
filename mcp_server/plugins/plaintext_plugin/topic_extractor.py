...
class TopicExtractor:
    ...
    def __init__(self):
        ...
        self.stop_words = {
            ...
            'that', 'the', 'to', 'was', 'will', 'with', 'would', 'could',
            'should', 'may', 'might', 'must', 'shall', 'can', 'need', 'dare',
            'ought', 'used', 'had', 'having', 'do', 'does', 'did', 'doing',
            'i', 'you', 'he', 'she', 'it', 'we', 'they', 'them', 'their',
            'this', 'that', 'these', 'those', 'am', 'is', 'are', 'was', 'were',
            'being', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would',
            'could', 'should', 'may', 'might', 'must', 'can', 'shall', 'ought',
            'need', 'dare', 'used', 'a', 'an', 'the', 'and', 'or', 'but', 'if',
        ...
        for seed_keyword in keywords:
            ...
            if seed_keyword in cooccurrence:
                related_scores = sorted(
                    cooccurrence[seed_keyword].items(),
                    key=lambda x: x[1],
                    reverse=True
                )
            ...
            if len(related) >= 2:  # Only create topic if we have related terms
                ...
                topic_score = sum(
                    next((score for kw, score in self.extract_keywords(text, 50) if kw == k), 0)
