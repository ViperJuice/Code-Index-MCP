...
class ParagraphDetector:
    ...
    def detect_paragraphs(self, text: str) -> List[Paragraph]:
        ...
        for i, line in enumerate(lines):
            ...
            if not current_para_lines:
                ...
                if self._should_start_new_paragraph(current_para_lines, line, current_indent, line_indent):
    ...
    def _is_heading(self, line: str, line_num: int, all_lines: List[str]) -> bool:
        ...
        if line_num + 1 < len(all_lines):
            ...
            if self.heading_patterns[2].match(next_line) or self.heading_patterns[3].match(next_line):
                return True
    ...
    def _should_start_new_paragraph(self, current_lines: List[str], new_line: str, 
                                   current_indent: int, new_indent: int) -> bool:
        ...
        if current_lines and self._is_list_item(current_lines[0]) and not self._is_list_item(new_line):
            return True
            
        return False
    
    def merge_related_paragraphs(self, paragraphs: List[Paragraph], 
                                max_distance: int = 1) -> List[Paragraph]:
        ...
        while i < len(paragraphs):
            ...
            while j < len(paragraphs) and j - i <= max_distance:
                ...
                if (not next_para.is_code_block and 
                    not self._is_heading(next_para.text, 0, [next_para.text]) and
                    abs(current.indent_level - next_para.indent_level) <= 2 and
                    current.is_list_item == next_para.is_list_item):
