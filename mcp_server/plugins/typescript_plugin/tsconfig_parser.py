...
class TSConfigParser:
        ...
        for pattern, mapped_paths in paths.items():
            if self._matches_pattern(module_name, pattern):
                for mapped_path in mapped_paths:
                    resolved = self._resolve_mapped_path(module_name, pattern, mapped_path, base_path)
    ...
    def _resolve_mapped_path(self, module_name: str, pattern: str, mapped_path: str, base_path: Path) -> Optional[Path]:
        ...
        if "*" in pattern and "*" in mapped_path:
            ...
            if module_name.startswith(prefix) and module_name.endswith(suffix):
                wildcard = module_name[len(prefix):len(module_name)-len(suffix) if suffix else len(module_name)]
    ...
    def _resolve_node_module(self, module_name: str, start_dir: Path, base_path: Path) -> Optional[Path]:
