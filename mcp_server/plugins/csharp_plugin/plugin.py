...
class Plugin(PluginWithSemanticSearch):
    ...
    def supports(self, path: str | Path) -> bool:
        ...
        path_obj = Path(path)
        return path_obj.suffix.lower() in {'.cs', '.csx', '.csproj', '.props', '.targets'}
                    ...
                    if file_id:
                        try:
                            ...
                            self._indexer.add_symbol(
                                ...
                                str(path),
                                symbol['line'],
                                {"symbol_id": symbol_id, "file_id": file_id, "kind": symbol['kind']}
                            )
                        except Exception as e:
        ...
        if type_info.get('is_generic') and type_info.get('generic_parameters'):
            ...
            for param in type_info['generic_parameters']:
                ...
                if param.get('constraints'):
                    constraints = [c['type'] if c['type'] != 'type' else c['value'] 
    ...
    def _extract_basic_symbols(self, content: str) -> List[Dict[str, Any]]:
        ...
        patterns = [
            ...
            (r'^\s*(?:public|private|protected|internal)?\s*delegate\s+.+\s+(\w+)', 'delegate'),
            (r'^\s*(?:public|private|protected|internal)?\s*(?:static\s+)?(?:async\s+)?\w+\s+(\w+)\s*\(', 'method'),
            (r'^\s*(?:public|private|protected|internal)?\s*\w+\s+(\w+)\s*\{', 'property'),
        ]
            ...
            for type_info in type_analysis.get('types', []):
                if type_info['name'] == symbol:
                    return SymbolDef(
                        ...
                        line=type_info['line'],
                        span=(type_info['line'], type_info.get('end_line', type_info['line']))
                    )
        ...
        for path in Path(".").rglob("*.cs"):
            try:
                ...
                if symbol in content:
                    ...
                    lines = content.split('\n')
                    for i, line in enumerate(lines):
                        if f" {symbol}" in line and any(keyword in line for keyword in 
