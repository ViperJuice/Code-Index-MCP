...
class NuGetIntegration:
    ...
    def _parse_package_metadata(self, nuspec_path: Path, package_name: str, version: str) -> PackageInfo:
    ...
    def _get_common_types_for_namespace(self, namespace: str) -> Set[str]:
        ...
        common_types = {
            'System': {
                'String', 'Int32', 'Int64', 'Boolean', 'DateTime', 'Guid',
                'Object', 'Exception', 'Type', 'Array', 'Enum', 'Delegate'
            },
            'System.Collections.Generic': {
                'List<T>', 'Dictionary<TKey, TValue>', 'IEnumerable<T>',
                'ICollection<T>', 'IList<T>', 'Queue<T>', 'Stack<T>',
                'HashSet<T>', 'SortedDictionary<TKey, TValue>'
            },
            'System.Linq': {
                'Enumerable', 'IQueryable<T>', 'IGrouping<TKey, TElement>'
            },
            'System.Threading.Tasks': {
                'Task', 'Task<T>', 'TaskCompletionSource<T>', 'CancellationToken'
            },
            'Microsoft.EntityFrameworkCore': {
                'DbContext', 'DbSet<T>', 'DbContextOptions', 'Entity'
            },
            'Microsoft.AspNetCore.Mvc': {
                'Controller', 'ControllerBase', 'ActionResult', 'IActionResult'
            },
            'Newtonsoft.Json': {
                'JsonConvert', 'JsonSerializer', 'JsonProperty', 'JsonIgnore'
            }
        }
        
        return common_types.get(namespace, set())
    
    def check_package_compatibility(self, package_name: str, version: str, target_framework: str) -> bool:
