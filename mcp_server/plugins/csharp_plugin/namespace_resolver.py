...
class NamespaceResolver:
    ...
    def _extract_declared_types(self, content: str) -> List[Dict[str, any]]:
        ...
        patterns = [
            (r'(?:public|private|protected|internal|static|abstract|sealed|partial)?\s*class\s+([A-Za-z_][A-Za-z0-9_]*)', 'class'),
            (r'(?:public|private|protected|internal|static|abstract|sealed|partial)?\s*interface\s+([A-Za-z_][A-Za-z0-9_]*)', 'interface'),
            (r'(?:public|private|protected|internal|static|partial)?\s*struct\s+([A-Za-z_][A-Za-z0-9_]*)', 'struct'),
            (r'(?:public|private|protected|internal|static)?\s*enum\s+([A-Za-z_][A-Za-z0-9_]*)', 'enum'),
            (r'(?:public|private|protected|internal|static)?\s*delegate\s+[^(]+\s+([A-Za-z_][A-Za-z0-9_]*)', 'delegate'),
            (r'(?:public|private|protected|internal|static|partial)?\s*record\s+([A-Za-z_][A-Za-z0-9_]*)', 'record')
        ]
        ...
        for using_ns in file_info['using_statements']:
            ...
            if using_ns == 'System':
                available_types.extend([
                    'String', 'Int32', 'Boolean', 'DateTime', 'Guid',
                    'Object', 'Exception', 'Type', 'Array', 'List'
                ])
            elif using_ns == 'System.Collections.Generic':
                available_types.extend([
                    'List<T>', 'Dictionary<TKey, TValue>', 'IEnumerable<T>',
                    'ICollection<T>', 'Queue<T>', 'Stack<T>'
                ])
