...
class PluginDiscovery(IPluginDiscovery):
    ...
    def discover_plugins(self, plugin_dirs: List[Path]) -> List[PluginInfo]:
        ...
        for plugin_dir in plugin_dirs:
            if (plugin_dir.exists() and 
                plugin_dir.name != "plugins" and  # Skip the main plugins directory
    ...
    def _extract_plugin_info_from_source(self, source: str, plugin_path: Path) -> Optional[PluginInfo]:
                    ...
                    if dict_start != -1:
                        ...
                        if brace_count == 0:
                            ...
                            try:
                                ...
                                info = {}
                                for key in ['name', 'version', 'description', 'author', 'language']:
                                    import re
                                    pattern = rf'"{key}"\s*:\s*"([^"]+)"'
                                ...
                                if 'name' in info:
                                    return PluginInfo(
                                        ...
                                        file_extensions=self._get_extensions_from_language(info.get('language', '')),
                                        path=plugin_path,
                                        module_name=f"mcp_server.plugins.{plugin_path.name}",
                                        class_name="Plugin"
                                    )
