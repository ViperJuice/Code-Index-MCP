...
class HybridSearch:
    ...
    def _initialize_reranker(self):
        ...
        try:
            ...
            self.reranker = factory.create_reranker(
                self.reranking_settings.reranker_type,
                config
            )
    ...
    async def _search_bm25(self, query: str, filters: Optional[Dict[str, Any]]) -> List[SearchResult]:
        """Perform BM25 search."""
    ...
    async def _search_semantic(self, query: str, filters: Optional[Dict[str, Any]]) -> List[SearchResult]:
    ...
    async def _search_fuzzy(self, query: str, filters: Optional[Dict[str, Any]]) -> List[SearchResult]:
    ...
    async def _rerank_results(self, query: str, results: List[SearchResult]) -> List[SearchResult]:
        ...
        try:
            ...
            # Perform reranking
            top_k = self.reranking_settings.top_k if self.reranking_settings else len(reranker_results)
            ...
            for rerank_item in rerank_data.results:
                ...
                if rerank_item.original_rank < 0 or rerank_item.original_rank >= len(results):
                    logger.warning(f"Invalid original_rank {rerank_item.original_rank} for {len(results)} results")
                    continue
            ...
            if hasattr(rerank_data, 'metadata') and rerank_data.metadata:
                logger.debug(f"Reranked {len(reranked_results)} results using {rerank_data.metadata.get('reranker', 'unknown')} reranker")
            else:
...
class HybridSearchOptimizer:
    ...
    def record_feedback(self, query: str, selected_result: int, results: List[Dict[str, Any]]):
        ...
        feedback = {
            ...
            'selected_source': results[selected_result]['source'] if selected_result < len(results) else None,
    ...
    def optimize_weights(self):
        ...
        if total_selections > 0:
            ...
            logger.info(f"Optimized weights - BM25: {bm25_weight:.3f}, "
    ...
    def get_optimization_report(self) -> Dict[str, Any]:
        ...
        if self.performance_history:
            avg_search_time = sum(p['search_time_ms'] for p in self.performance_history) / len(self.performance_history)
