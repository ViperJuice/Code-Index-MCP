...
class FileTypeMatcher(IFileTypeMatcher):
    ...
    def get_file_info(self, path: Path) -> FileTypeInfo:
        ...
        if (path_str in self._cache and 
            ...
            current_time - self._cache_timestamps[path_str] < self._cache_timeout):
...
class PluginRouter(IPluginRouter):
    ...
    def register_plugin(self, plugin: IPlugin, capabilities: Optional[List[PluginCapability]] = None) -> None:
    ...
    def _get_plugin_extensions(self, plugin: IPlugin) -> Set[str]:
        ...
        test_extensions = [
            ...
            '.java', '.go', '.rs', '.rb', '.php', '.swift', '.dart', '.kt',
            '.html', '.css', '.scss', '.xml', '.json', '.yaml', '.md',
            '.sh', '.sql', '.lua', '.pl', '.r', '.scala', '.clj'
        ]
    ...
    def route_file(self, path: Path) -> List[RouteResult]:
        ...
        results.sort(key=lambda r: (r.confidence, -self._plugin_usage_count[r.plugin]), reverse=True)
    ...
    def route_by_capability(self, capability: str, **kwargs) -> List[RouteResult]:
        ...
        results.sort(key=lambda r: (r.confidence, max((cap.priority for cap in r.capabilities), default=0)), reverse=True)
    ...
    def _calculate_confidence(self, plugin: IPlugin, path: Path, file_info: FileTypeInfo) -> Tuple[float, List[str]]:
    ...
    def get_plugin_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'plugin_performance': {
                plugin: self._get_avg_performance(plugin)
    ...
    def configure(self, load_balance_enabled: bool = True, 
                 performance_tracking_enabled: bool = True,
                 max_performance_samples: int = 100) -> None:
