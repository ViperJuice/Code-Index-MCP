...
from mcp_server.core.errors import DocumentProcessingError
...
With some concluding text."""
        ...
        result = markdown_plugin.indexFile(str(corrupted_file))
    ...
    def test_encoding_fallback_chain(self, plaintext_plugin, tmp_path):
        ...
        assert result["metadata"]["final_encoding"] in ["latin-1", "windows-1252", "utf-8"]
...
### Final valid section"""
        ...
        result = markdown_plugin.indexFile(str(malformed_file))
...
that continues..."""
        ...
        for detail_level in ["minimal", "standard", "verbose"]:
            with patch.object(markdown_plugin, 'error_detail_level', detail_level):
                result = markdown_plugin.indexFile(str(error_file))
