...
from mcp_server.dispatcher import (
    PluginRouter, FileTypeMatcher, ResultAggregator,
    IPluginRouter, IFileTypeMatcher, IResultAggregator,
    SimpleAggregationStrategy, SmartAggregationStrategy,
    AggregatedResult, AggregationStats, RankingCriteria,
    PluginCapability, FileTypeInfo, RouteResult
)
...
def python_plugin():
    ...
    plugin._references = {
        'hello_world': [
            Reference(file='/test/file3.py', line=5)
        ]
    }
    return plugin
...
class TestResultAggregator:
    ...
    def test_aggregate_search_results_basic(self, result_aggregator, python_plugin, javascript_plugin, sample_search_results):
    ...
    def test_aggregate_search_results_with_limit(self, result_aggregator, python_plugin, sample_search_results):
    ...
    def test_aggregate_symbol_definitions(self, result_aggregator, python_plugin, javascript_plugin):
        ...
        definitions_by_plugin = {
            python_plugin: python_def,
            javascript_plugin: js_def
        }
    ...
    def test_aggregate_references(self, result_aggregator, python_plugin, javascript_plugin):
        ...
        references_by_plugin = {
            python_plugin: python_refs,
            javascript_plugin: js_refs
        }
...
class TestAggregationStrategies:
    ...
    def test_smart_aggregation_strategy(self, python_plugin, javascript_plugin, sample_search_results):
...
class TestIntegration:
    ...
    def test_end_to_end_search_flow(self, plugin_router, python_plugin, javascript_plugin):
        ...
        # Index some content
        python_plugin.indexFile('/test/example.py', 'def hello_world():\n    print("Hello, World!")')
        javascript_plugin.indexFile('/test/app.js', 'function hello() {\n    console.log("Hello!");\n}')
        ...
        for plugin in plugin_router._plugins:
            ...
            if any(cap.name == 'semantic_search' for cap in capabilities) or True:  # All plugins can search
        ...
        assert all(query.lower() in result.primary_result['snippet'].lower() for result in aggregated)
