...
class TestRepositoryIndexing:
    ...
    @pytest.mark.parametrize("repo_name,repo_url,expected_files,expected_symbols", [
        ("requests", "https://github.com/psf/requests.git", 50, 500),
        ("terminal_subset", "https://github.com/microsoft/terminal.git", 200, 2000),
    ])
    def test_small_repository_indexing(self, repo_name, repo_url, expected_files, expected_symbols, 
                                     workspace_dir, test_db, dispatcher, benchmark):
        ...
        # Validate results
        assert indexed_files >= expected_files * 0.6, f"Expected at least {expected_files * 0.6} files, got {indexed_files}"
        assert total_symbols >= expected_symbols * 0.6, f"Expected at least {expected_symbols * 0.6} symbols, got {total_symbols}"
    ...
    def test_large_repository_indexing(self, workspace_dir, test_db, dispatcher, benchmark):
        ...
        assert error_count < indexed_files * 0.1, f"Error rate too high: {error_count}/{indexed_files}"
    ...
    def test_multi_language_repository(self, workspace_dir, test_db, dispatcher):
        ...
        repo_id = test_db.create_repository(str(repo_path), "vscode_subset", {"type": "multi_lang_test"})
        ...
        languages_with_results = [lang for lang, stats in language_stats.items() if stats['indexed'] > 0]
        assert len(languages_with_results) >= 2, f"Should support multiple languages, got: {languages_with_results}"
        ...
        for language, stats in language_stats.items():
            if stats['files'] > 0:
                ...
                print(f"{language}: {stats['indexed']}/{stats['files']} files ({success_rate:.1%}), {stats['symbols']} symbols")
    ...
    def test_indexing_memory_efficiency(self, workspace_dir, test_db, dispatcher):
        ...
        repo_id = test_db.create_repository(str(repo_path), "requests_memory_test", {"type": "memory_test"})
        ...
        print(f"Memory test: {indexed_files} files, {memory_increase:.1f}MB increase, {max_memory:.1f}MB peak")
