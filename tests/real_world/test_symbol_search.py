...
class TestSymbolSearch:
    ...
    def search_code(self, query: str, test_db: SQLiteStore, repo_id: int, limit: int = 20) -> List[Dict]:
    ...
    @pytest.mark.parametrize("repo_name,repo_url,search_terms", [
        ("requests", "https://github.com/psf/requests.git", 
         ["Session", "get", "post", "Response", "HTTPAdapter"]),
        ("django_subset", "https://github.com/django/django.git", 
         ["Model", "QuerySet", "CharField", "ForeignKey", "admin"]),
    ])
    def test_real_world_symbol_search(self, repo_name, repo_url, search_terms, 
                                    workspace_dir, test_db, dispatcher, fuzzy_indexer, benchmark):
        ...
        assert found_symbols >= len(search_terms) * 0.6, f"Symbol discovery rate too low: {found_symbols}/{len(search_terms)}"
        ...
        for term, result in results.items():
            print(f"  {term}: {'✓' if result['symbol_found'] else '✗'} "
    ...
    def test_cross_language_search(self, workspace_dir, test_db, dispatcher, fuzzy_indexer):
        ...
        repo_id = self.setup_indexed_repository(
            workspace_dir, "vscode_search", "https://github.com/microsoft/vscode.git",
        ...
        for case in test_cases:
            ...
            assert len(all_results) >= 3, f"Expected multiple results for '{term}', got {len(all_results)}"
            assert len(languages_found) >= 1, f"Expected multi-language results for '{term}', got {languages_found}"
            print(f"Term '{term}' found in languages: {languages_found} ({len(all_results)} results)")
    
    @pytest.mark.performance
    def test_symbol_lookup_performance(self, workspace_dir, test_db, dispatcher, fuzzy_indexer, benchmark):
        ...
        for size, stats in performance_results.items():
            print(f"  {size}: {stats['avg_time_ms']:.1f}ms avg, "
                  f"{stats['max_time_ms']:.1f}ms max, "
                  f"{stats['success_rate']:.1%} success")
    
    @pytest.mark.performance
    @pytest.mark.asyncio
    async def test_concurrent_search_performance(self, workspace_dir, test_db, dispatcher, fuzzy_indexer):
        """Test performance under concurrent search queries."""
        repo_id = self.setup_indexed_repository(
            workspace_dir, "concurrent_test", "https://github.com/psf/requests.git",
            test_db, dispatcher, fuzzy_indexer
        )
    ...
    def get_common_symbols_for_repo(self, repo_name: str) -> List[str]:
        ...
        symbol_map = {
            "requests": ["Session", "Response", "get", "post", "HTTPAdapter", "Request"],
            "django": ["Model", "QuerySet", "CharField", "ForeignKey", "admin", "forms"],
    ...
    def test_search_result_quality(self, workspace_dir, test_db, dispatcher, fuzzy_indexer):
        ...
        repo_id = self.setup_indexed_repository(
            workspace_dir, "quality_test", "https://github.com/psf/requests.git",
            test_db, dispatcher, fuzzy_indexer
        )
