...
class TestRedisCaching:
    ...
    async def test_cache_invalidation_patterns(self, redis_cache_manager, query_cache):
        ...
        # Set up test data
        test_data = {
            "file1.py": {"symbols": ["ClassA", "function_b"], "content": "class ClassA: pass"},
            "file2.py": {"symbols": ["ClassC", "function_d"], "content": "def function_d(): pass"},
            "shared.py": {"symbols": ["SharedClass"], "content": "class SharedClass: pass"}
        }
    ...
    async def test_cache_memory_efficiency(self, redis_cache_manager):
        ...
        print(f"Memory efficiency test: {entries_added} entries added, {retrieved_count} retrieved in {retrieval_time:.3f}s")
    ...
    async def test_cache_concurrency_safety(self, redis_cache_manager):
        ...
        assert successful_writes >= 55, f"Should complete most writes, completed {successful_writes}"
    ...
    async def test_cache_persistence_and_recovery(self, redis_cache_manager):
        ...
        try:
            ...
            assert recovered_count >= 2, f"Should recover most persistent data, recovered {recovered_count}"
    ...
    async def test_cache_hit_rate_optimization(self, redis_cache_manager, query_cache):
        ...
        common_queries = ["user authentication", "session management", "API endpoint", "database query"]
        ...
        for round_num in range(3):
            ...
            for query in common_queries:
                ...
                if cached_result is not None:
                    ...
                    results = [
                        {"file": f"{query.replace(' ', '_')}.py", "line": 1, "snippet": query}
                    ]
                    await query_cache.cache_result(
                        QueryType.SEARCH,
                        results,
        ...
        assert hit_rate >= 0.6, f"Cache hit rate {hit_rate:.2f} should be >= 0.6 for repeated queries"
...
class TestRedisCacheIntegration:
    ...
    async def test_cache_with_file_watcher_integration(self, integrated_cache_system):
        ...
        await cache_manager.set(f"file:{test_file}:metadata", {"size": 1024, "modified": time.time()}, ttl=600)
    ...
    async def test_cache_performance_under_load(self, integrated_cache_system, benchmark):
        ...
        print(f"Load test metrics: {metrics.hits} hits, {metrics.misses} misses, hit rate: {metrics.hit_rate:.2%}")
        
        # Should have reasonable hit rate due to repeated operations
        if metrics.hits + metrics.misses > 0:
            assert metrics.hit_rate >= 0.2, f"Hit rate {metrics.hit_rate:.2%} should be reasonable under load"