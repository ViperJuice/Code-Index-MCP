...
class TestMemoryUsage:
    ...
    def test_memory_usage_large_codebase(self, workspace_dir, test_db, dispatcher):
        ...
        repo_id = test_db.create_repository(str(repo_path), "linux_memory_test", {"type": "memory_test"})
        ...
        print(f"  Memory per file: {memory_increase / indexed_count * 1024:.1f}KB" if indexed_count > 0 else "  N/A")
        ...
        assert indexed_count >= len(c_files) * 0.7, f"Should index most files: {indexed_count}/{len(c_files)}"
        ...
        if len(memory_samples) > 4:
            ...
            assert growth_rate < 5, f"Possible memory leak detected: {growth_rate:.1f}MB/sample growth rate"
    ...
    def test_memory_leak_detection(self, workspace_dir, test_db, dispatcher):
        ...
        for cycle in range(5):
            ...
            if cycle > 2:
                assert cycle_increase < 30, f"Cycle {cycle}: Memory increase too high: {cycle_increase:.1f}MB"
                assert total_increase < 100, f"Cycle {cycle}: Total memory growth too high: {total_increase:.1f}MB"
        ...
        for sample in memory_samples:
            print(f"  Cycle {sample['cycle']}: +{sample['cycle_increase_mb']:.1f}MB "
        ...
        if len(memory_samples) >= 4:
            ...
            assert avg_late_increase < 15, f"Memory leak detected: avg late cycle increase {avg_late_increase:.1f}MB"
        
        assert final_memory_increase < 80, f"Total memory growth too high: {final_memory_increase:.1f}MB"
    ...
    def test_memory_efficiency_by_language(self, workspace_dir, test_db, dispatcher):
        ...
        test_repos = [
            ...
            ("js_memory", "https://github.com/facebook/react.git", ['.js', '.jsx'], "JavaScript"),
            ("cpp_memory", "https://github.com/microsoft/terminal.git", ['.cpp', '.hpp'], "C++")
        ]
        ...
        for repo_name, repo_url, extensions, language in test_repos:
            ...
            try:
                ...
                repo_id = test_store.create_repository(
                    str(repo_path), f"{repo_name}_efficiency", {"type": "efficiency_test"}
                )
                ...
                language_results[language] = {
                    ...
                    'memory_per_file_kb': (memory_increase * 1024) / indexed_count if indexed_count > 0 else 0,
                    'memory_per_symbol_bytes': (memory_increase * 1024 * 1024) / total_symbols if total_symbols > 0 else 0,
                    'memory_per_line_bytes': (memory_increase * 1024 * 1024) / total_lines if total_lines > 0 else 0
                }
        ...
        for language, result in language_results.items():
            print(f"  {language:10s}: {result['memory_increase_mb']:5.1f}MB total, "
                  f"{result['memory_per_file_kb']:5.1f}KB/file, "
                  f"{result['memory_per_symbol_bytes']:6.0f}B/symbol")
            
            # Language-specific assertions
            assert result['memory_increase_mb'] < 50, f"{language}: Memory usage too high: {result['memory_increase_mb']:.1f}MB"
            assert result['memory_per_file_kb'] < 200, f"{language}: Memory per file too high: {result['memory_per_file_kb']:.1f}KB"
            assert result['indexed_files'] >= len(files) * 0.7, f"{language}: Should index most files"
    ...
    def test_memory_usage_with_profiler(self, workspace_dir, dispatcher):
        ...
        def index_files_for_profiling():
            ...
            try:
                store = SQLiteStore(db_path)
                repo_id = store.create_repository(str(repo_path), "profiler_test", {"type": "profile_test"})
        ...
        assert max_memory < min_memory + 80, f"Peak memory usage too high: {max_memory:.1f}MB vs {min_memory:.1f}MB baseline"
            ...
            def worker():
                ...
                try:
                    ...
                    repo_id = store.create_repository(
                        str(repo_path), f"concurrent_test_{threading.current_thread().ident}", 
