...
from typing import Any, Dict, List, Optional, Tuple, TypeVar, Callable, Union
import threading
import psutil
import tracemalloc
...
class TokenUsage:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'cache_efficiency': self.prompt_cache_hits / (self.prompt_cache_hits + self.prompt_cache_misses)
                               if (self.prompt_cache_hits + self.prompt_cache_misses) > 0 else 0.0
        }
...
class BaseComparison(abc.ABC):
    ...
    def execute_batch(self, inputs: List[Any], expected: Optional[List[Any]] = None) -> List[ComparisonResult]:
...
class AsyncBaseComparison(BaseComparison):
    ...
    async def execute_batch_async(self, inputs: List[Any], expected: Optional[List[Any]] = None) -> List[ComparisonResult]:
