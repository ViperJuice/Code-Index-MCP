...
class TestCohereReranker:
    ...
    async def test_cohere_caching_preserves_metadata(self, cohere_reranker):
        ...
        mock_response = Mock()
        mock_response.results = [
            Mock(index=1, relevance_score=0.99)
        ]
        mock_client.rerank.return_value = mock_response
        ...
        with patch('cohere.Client', return_value=mock_client):
            ...
            assert cached_item.original_result.match_type == original_item.original_result.match_type
...
class TestHybridReranker:
    ...
    async def test_hybrid_primary_success(self, hybrid_reranker):
        ...
        primary.rerank = AsyncMock(return_value=Result.ok(
            RerankResult(
    ...
    async def test_hybrid_fallback_on_failure(self, hybrid_reranker):
        ...
        fallback.rerank = AsyncMock(return_value=Result.ok(
            RerankResult(
                results=fallback_items,
                metadata={'reranker': 'fallback'}
            )
        ))
    ...
    async def test_multiple_reranking_preserves_original(self):
        ...
        for item in result2.data.results:
            ...
            for original in original_results:
                if (original.file_path == item.original_result.file_path and
                    original.line == item.original_result.line):
                    original_found = True
