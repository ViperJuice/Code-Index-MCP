...
from tests.test_utils import (
    ...
    create_mock_search_results, assert_performance
)
...
class TestDocumentSearchPerformance(BaseDocumentTest):
    ...
    def setup_search_corpus(self, size: str = "medium") -> Dict[str, Any]:
        ...
        topics = [
            "API Documentation", "User Guide", "Installation",
            "Configuration", "Troubleshooting", "Best Practices",
            "Security", "Performance", "Architecture", "Deployment",
            "Testing", "Monitoring", "Scaling", "Migration", "Integration",
            "Authentication", "Authorization", "Logging", "Caching", "Optimization"
        ][:config["topics"]]
...
"""
    ...
    def test_search_query_latency(self):
        ...
        print(f"Corpus ready: {len(corpus['documents'])} documents indexed in {corpus['index_time']:.1f}s")
        ...
        test_queries = [
            ...
            ("long_query", "troubleshooting performance issues in production deployment with high load"),
        ]
        ...
        for query_type, query in test_queries:
            ...
            p99_time = statistics.quantiles(query_times, n=100)[98] if len(query_times) >= 20 else max(query_times)
            ...
            print(f"  Avg: {avg_time:.1f}ms, P50: {p50_time:.1f}ms, P95: {p95_time:.1f}ms, P99: {p99_time:.1f}ms")
    ...
    def test_query_optimization_effectiveness(self):
        ...
        for test in optimization_tests:
            ...
            p95_time = statistics.quantiles(search_times, n=20)[14] if len(search_times) >= 15 else max(search_times)
        ...
        for name, result in results.items():
            if name != "baseline":
                avg_improvement = ((baseline["avg_ms"] - result["avg_ms"]) / baseline["avg_ms"]) * 100
                p95_improvement = ((baseline["p95_ms"] - result["p95_ms"]) / baseline["p95_ms"]) * 100
                
                print(f"{name:<12} | {result['avg_ms']:>7.1f} | {result['p95_ms']:>7.1f} | "
    ...
    def test_pagination_performance(self):
        ...
        for size, data in results.items():
            print(f"{size:>9} | {data['avg_ms']:>7.1f} | {data['max_ms']:>7.1f} | {data['per_result_ms']:>9.2f}")
        ...
        assert time_increase < size_increase * 0.5, \
    ...
    def test_concurrent_search_throughput(self):
        ...
        for num_concurrent in concurrency_levels:
            ...
            results[num_concurrent] = {
                ...
                "p95_latency_ms": statistics.quantiles(query_times, n=20)[18] if len(query_times) >= 20 else max(query_times)
            }
        ...
        for level, data in results.items():
            ...
            print(f"{level:>10} | {data['qps']:>3.0f} | {data['avg_latency_ms']:>7.1f} | "
        ...
        assert results[max_concurrent]["p95_latency_ms"] < 1000, \
