...
from tests.test_utils import (
    memory_monitor, generate_large_content, create_test_markdown,
    create_test_plaintext, assert_memory_usage
)
...
class TestDocumentMemoryUsage(BaseDocumentTest):
    ...
    def test_memory_profile_document_processing(self):
        ...
        for name, data in results.items():
            print(f"{name:<6} | {data['size_mb']:>7.1f} | {data['memory_used_mb']:>7.2f} | {data['memory_ratio']:>5.2f}")
    ...
    def test_memory_leak_detection(self):
        ...
        assert growth < 5.0, f"Potential memory leak: {growth:.2f}MB growth over {num_iterations} iterations"
    ...
    def test_resource_cleanup_effectiveness(self):
        ...
        cleanup_ratio = (phase1_memory - phase2_memory) / (phase1_memory - profiler.initial_memory["rss"])
    ...
    def test_memory_usage_under_concurrent_load(self):
        ...
        for num_workers in worker_counts:
            ...
            # Get memory stats
            start_checkpoint = next(s for s in profiler.snapshots if s["label"] == f"workers_{num_workers}_start")
            end_checkpoint = next(s for s in profiler.snapshots if s["label"] == f"workers_{num_workers}_end")
            cleanup_checkpoint = next(s for s in profiler.snapshots if s["label"] == f"workers_{num_workers}_cleanup")
            
            memory_used = end_checkpoint["memory"]["rss"] - start_checkpoint["memory"]["rss"]
            memory_after_cleanup = cleanup_checkpoint["memory"]["rss"] - start_checkpoint["memory"]["rss"]
        ...
        for workers, data in results.items():
            print(f"{workers:>7} | {data['memory_peak_mb']:>8.1f} | {data['memory_after_cleanup_mb']:>11.1f} | "
        ...
        if len(results) >= 2:
            ...
            mem_ratio = results[worker_list[-1]]["memory_peak_mb"] / results[worker_list[0]]["memory_peak_mb"]
    ...
    def test_memory_pressure_handling(self):
            ...
            for i in range(max_documents):
                ...
                try:
                    ...
                    if cleanup_checkpoint["memory"]["percent"] > 80:
                        print(f"  High memory usage: {cleanup_checkpoint['memory']['percent']:.1f}%")
        ...
        recovery_ratio = memory_recovered / (profile_summary['peak_mb'] - profile_summary['initial_mb'])
