...
class TestDocumentIndexingPerformance(BaseDocumentTest):
    ...
    def test_batch_indexing_throughput(self):
        ...
        for batch_size in batch_sizes:
            ...
            if batch_size >= 100:
                assert (
                    throughput >= 100
                ), f"Throughput too low: {throughput:.1f} docs/s (need >= 100)"
    ...
    def test_large_file_indexing_performance(self):
        ...
        with memory_monitor():
            for size_mb in file_sizes_mb:
                ...
                assert (
                    size_mb / index_time
                ) >= 1.0, f"Processing too slow: {size_mb/index_time:.1f} MB/s"
        ...
        for size, data in results.items():
            ...
            print(
                f"{size:>8.1f} | {data['index_time_s']:>7.2f} | {rate:>4.1f} | {data['ms_per_mb']:>5.0f}"
            )
    ...
    def test_memory_usage_during_indexing(self):
        ...
        if len(memory_results) >= 2:
            ...
            mem_ratio = (
                memory_results[counts[-1]]["memory_used_mb"]
                / memory_results[counts[0]]["memory_used_mb"]
            )
