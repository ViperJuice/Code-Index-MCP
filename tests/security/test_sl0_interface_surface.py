...
def test_capability_set_defaults():
    dc_fields = {
        ...
        for f in dataclasses.fields(
            __import__("mcp_server.sandbox.capabilities", fromlist=["CapabilitySet"]).CapabilitySet
        )
    }
...
def test_attest_skip_mode_returns_attestation():
    ...
    from mcp_server.artifacts.attestation import Attestation, attest
...
def test_verify_attestation_skip_mode_no_ops():
    ...
    from mcp_server.artifacts.attestation import Attestation, verify_attestation

    att = Attestation(
        bundle_url="", bundle_path=Path(""), subject_digest="", signed_at=datetime.now(timezone.utc)
    )
    with patch.dict(os.environ, {"MCP_ATTESTATION_MODE": "skip"}):
