...
class TestMemoryAwarePluginManager:
    ...
    def test_memory_limit_enforcement(self, manager):
        ...
        with patch.object(manager, '_get_plugin_memory_usage', return_value=manager.max_memory_bytes + 1):
    ...
    def test_high_priority_protection(self, manager):
        ...
        with patch.object(manager._factory, 'get_plugin', side_effect=[python_plugin, other_plugin]):
            # Load both plugins
    ...
    def test_plugin_eviction(self, manager):
        ...
        # Add plugin manually
        manager._plugins['test'] = LoadedPlugin(
            name='test',
            instance=plugin,
            metadata={}
        )
    ...
    def test_clear_cache(self, manager):
        ...
        for lang in ['python', 'rust', 'go']:
            plugin = Mock()
            manager._plugins[lang] = LoadedPlugin(
                name=lang,
                instance=plugin,
                metadata={}
            )
...
class TestMemoryMonitoring:
    ...
    def test_weak_reference_cleanup(self):
        ...
        manager._weak_refs['test'] = weakref.ref(
            plugin,
            lambda ref: manager._on_plugin_deleted('test')
        )
