...
class TestComplexCodeStructures:
    ...
    def test_function_pointers_and_callbacks(self):
        ...
        assert any(s["symbol"] == "get_default_handler" and s["kind"] == "function" for s in symbols)
...
class TestSearchFunctionality:
    ...
    def test_get_definition(self, sqlite_store):
        ...
        file_id = sqlite_store.store_file(repo_id, "/test/file.c", "file.c", language="c")
    ...
    def test_find_references(self):
        ...
        plugin._parsed_files["main.c"] = (dedent("""
        #include "math.h"
        ...
        plugin._parsed_files["math.c"] = (dedent("""
        int add(int a, int b) {
            return a + b;
        }
...
""")
        ...
        for filename, code in test_files:
            file_id = sqlite_store.store_file(
                repo_id, f"/test/{filename}", filename, language="c"
            )
...
class TestPersistenceIntegration:
    ...
    def test_full_indexing_with_persistence(self, sqlite_store):
        ...
        file_id = sqlite_store.store_file(
            repo_id, str(file_path), "main.c", 
            language="c", size=len(code)
        )
