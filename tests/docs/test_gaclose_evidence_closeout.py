...
def test_active_public_release_surfaces_are_rc5_and_not_latest_driven():
    failures = []
    for relative in PUBLIC_SURFACES:
        text = _read(relative)
        if PUBLIC_ALPHA_VERSION not in text:
            failures.append(f"{relative}: missing {PUBLIC_ALPHA_VERSION}")
