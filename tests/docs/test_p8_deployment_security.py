...
def test_existing_auth_subsection_unchanged():
    ...
    assert (
        "### 1. Authentication and Authorization" in headings
...
def test_mcp_access_controls_subsection_present():
    ...
    headings = [l for l in lines if l.startswith("### ")]
    assert (
        "### MCP Access Controls" in headings
...
def test_mcp_access_controls_inside_security_section():
    ...
    assert any(
        l == "### MCP Access Controls" for l in section_lines
