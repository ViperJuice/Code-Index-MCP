...
class TestHTMLCSSPlugin:
    ...
    def test_index_html_with_classes(self, plugin):
        ...
        expected = {".container", ".text-primary", ".bold", ".highlight", ".row", ".flex-container"}
    ...
    def test_index_css_complex_selectors(self, plugin):
        ...
        assert any(s["symbol"] == ".button:active" and s["kind"] == "pseudo-selector" for s in symbols)
        assert any(s["symbol"] == '[data-toggle="modal"]' and s["kind"] == "attribute-selector" for s in symbols)
    ...
    def test_cross_references(self, plugin):
        ...
        # Index files
        plugin.indexFile("page.html", """
        <div class="alert alert-danger">Error message</div>
        <div class="alert alert-success">Success message</div>
        """)
        
        plugin.indexFile("alerts.css", """
        .alert {
            padding: 15px;
            margin-bottom: 20px;
