...
from mcp_server.storage.repository_registry import RepositoryRegistry, RepositoryInfo
...
class TestRepositoryManagement:
    ...
    def test_register_single_repository(self, test_env, registry):
        ...
        repo = TestRepositoryBuilder.create_repository(
            test_env, "test_repo", language="python"
        )
    ...
    def test_register_multiple_repositories(self, test_env, registry):
        ...
        for i in range(5):
            ...
            repo = TestRepositoryBuilder.create_repository(
                test_env, f"repo_{i}", language=lang
            )
            repos.append(repo)
    ...
    def test_repository_discovery(self, test_env, registry):
        ...
        repos.append(TestRepositoryBuilder.create_repository(
            workspace, "top_level", language="python"
        ))
        repos.append(TestRepositoryBuilder.create_repository(
    ...
    def test_repository_status_tracking(self, test_env, registry):
        ...
        repo = TestRepositoryBuilder.create_repository(
            test_env, "status_repo", language="python"
        )
        ...
        TestRepositoryBuilder.run_git_command(
            "git commit -m 'Add new feature'", repo.path
        )
    ...
    def test_auto_sync_configuration(self, test_env, registry):
        ...
        all_repos = registry.get_all_repositories()
        auto_sync_repos = {
            rid: info for rid, info in all_repos.items() 
            if info.auto_sync
        }
    ...
    def test_repository_path_lookup(self, test_env, registry):
        ...
        repo = TestRepositoryBuilder.create_repository(
            test_env, "lookup_repo", language="go"
        )
        repo_id = registry.register_repository(str(repo.path))
