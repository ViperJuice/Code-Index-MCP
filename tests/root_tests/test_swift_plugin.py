...
"""
    ...
    try:
        ...
        property_wrappers = protocol_checker.analyze_property_wrappers(
            swift_content, str(temp_file)
        )
        ...
        assert (
            "State" in wrapper_names
        ), f"Should find State property wrapper, found: {wrapper_names}"
        ...
        assert (
            "ViewBuilder" in builder_names
        ), f"Should find ViewBuilder result builder, found: {builder_names}"
