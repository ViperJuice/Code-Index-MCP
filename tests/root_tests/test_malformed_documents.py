...
function process() {
    ...
    def test_binary_content_in_text_files(self, setup_plugins):
        ...
        result1 = plugins["markdown"].indexFile(
            "binary.md", binary_md.decode("utf-8", errors="replace")
        )
        assert result1 is not None

        # Binary content in plaintext
        binary_txt = b"Some text\x00\x00\x00More text\xff\xff\xff"
        result2 = plugins["plaintext"].indexFile(
            "binary.txt", binary_txt.decode("utf-8", errors="replace")
        )
        assert result2 is not None
