...
class TestChunkOptimizer:
    ...
    def setup_method(self):
        ...
        self.config = ChunkingConfig(
            strategy=ChunkingStrategy.HYBRID, max_chunk_size=100, min_chunk_size=20, overlap_size=10
        )
    ...
    def test_calculate_optimal_chunk_size(self):
        ...
        size_with_structure = self.optimizer.calculate_optimal_chunk_size(long_content, structure)
...
Another paragraph with different content."""
        ...
        assert any(
            "\n" in text[max(0, point - 1) : min(len(text), point + 1)] for point in split_points
        )
...
class TestChunkingStrategies:
    ...
    def setup_method(self):
        ...
        self.structure = DocumentStructure(
            title="Test Document", sections=[], metadata={"path": "test.txt"}
        )
    ...
    def test_fixed_size_strategy(self):
        ...
        if len(chunks) > 1:
            ...
            assert any(
                chunks[i].content[-5:] in chunks[i + 1].content[:20] for i in range(len(chunks) - 1)
            )
...
In conclusion, we summarize."""
    ...
    def test_hybrid_strategy(self):
        ...
        section1 = Section(
            id="1", heading="Introduction", level=1, content="This is the introduction section."
        )
        section2 = Section(
            id="2",
...
class TestChunkOptimizerFactory:
    ...
    def test_create_optimizer_with_custom_config(self):
        ...
        optimizer, strategy = create_chunk_optimizer(ChunkingStrategy.SENTENCE_BASED, custom_config)
...
class TestEdgeCases:
    ...
    def setup_method(self):
        ...
        self.structure = DocumentStructure(title="Test", sections=[], metadata={})
    ...
    def test_malformed_structure(self):
        ...
        section = Section(id="1", heading="Circular", level=1, content="Content")
        section.parent = section  # Circular reference

        bad_structure = DocumentStructure(title="Bad", sections=[section], metadata={})
