...
This script implements the parallel testing plan defined in COMPREHENSIVE_PARALLEL_TESTING_PLAN.md,
...
class ParallelTestRunner:
                ...
                test_groups=[
                    {
                        ...
                        "command": "pytest tests/test_gateway.py -n 4 --dist=loadscope --tb=short",
                        "workers": 4,
                    },
                    {
    ...
    async def run_phase(self, phase: TestPhase) -> List[TestResult]:
        ...
        print(
            f"📊 Running {len(phase.test_groups)} test groups with {phase.parallel_workers} workers"
        )
        ...
        async def run_with_semaphore(group):
            async with semaphore:
                return await self.run_test_command(group["name"], group["command"], phase.name)
        ...
        print(
            f"📈 Phase {phase.name} complete: {successful}/{total} groups passed in {total_duration:.1f}s"
        )
    ...
    def generate_report(self) -> Dict:
        ...
        for phase_name in set(r.phase for r in self.results):
            ...
            phases_summary[phase_name] = {
                ...
                "total_duration": sum(r.duration_seconds for r in phase_results),
                "average_duration": (
                    sum(r.duration_seconds for r in phase_results) / len(phase_results)
                    if phase_results
                    else 0
                ),
            }
    ...
    def print_summary(self, report: Dict):
        ...
        print(
            f"📊 Test Groups: {summary['successful_groups']}/{summary['total_groups']} passed ({summary['success_rate_percent']:.1f}%)"
        )
        ...
        for phase_name, phase_data in report["phases_summary"].items():
            ...
            print(
                f"  {status_emoji} {phase_name}: {phase_data['successful_groups']}/{phase_data['total_groups']} groups ({phase_data['total_duration']:.1f}s)"
            )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run comprehensive parallel tests for Code-Index-MCP"
    )
    parser.add_argument(
        "--phases",
        nargs="+",
    ...
    parser.add_argument(
        "--setup-only", action="store_true", help="Only setup environment, don't run tests"
    )
    parser.add_argument(
        "--setup-real-world",
        action="store_true",
        help="Setup real-world testing environment and download repositories",
    )
    ...
    if args.setup_real_world:
        ...
        try:
            ...
            subprocess.run(
                [sys.executable, "scripts/download_test_repos.py", "--tier", "tier1"], check=True
            )
