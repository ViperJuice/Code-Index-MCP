...
class TestSemanticDocumentIntegration(BaseDocumentTest):
    ...
    def semantic_indexer(self, mock_qdrant_client, mock_voyage_client):
        ...
        with patch.dict(os.environ, {"VOYAGE_API_KEY": "test-key"}):
            indexer = SemanticIndexer(
                collection_name="test_collection", qdrant_host="localhost", qdrant_port=6333
            )
            return indexer
    ...
    def test_semantic_search_with_document_context(self, semantic_indexer, mock_qdrant_client):
        ...
        results = semantic_indexer.search_documents(
            query="how to install with pip", limit=10, file_pattern="*.md"
        )
...
"""
        ...
        with patch.dict(os.environ, {"SEMANTIC_SEARCH_ENABLED": "true"}):
            plugin = MarkdownPlugin(sqlite_store=self.store, enable_semantic=True)
    ...
    def test_semantic_search_respects_file_patterns(self, semantic_indexer, mock_qdrant_client):
        ...
        md_results = semantic_indexer.search_documents(
            query="configuration", limit=10, file_pattern="*.md"
        )
...
"""
        ...
        semantic_indexer.index_document_content(
            content=large_content, file_path=str(self.workspace / "large.md"), doc_type="markdown"
        )
    ...
    def test_semantic_search_with_synonyms(self, semantic_indexer, mock_qdrant_client):
        ...
        results = semantic_indexer.search_documents(
            query="software deployment setup", limit=10  # Mix of synonyms
        )
...
"""
        ...
        # Index document
        semantic_indexer.index_document_content(
            document, str(self.workspace / "manual.md"), "markdown"
        )
    ...
    def test_semantic_vector_search_accuracy(self, semantic_indexer, mock_qdrant_client):
        ...
        test_docs = [
            ...
            {"id": "similar", "content": "Database configuration and setup guide", "score": 0.92},
            {"id": "related", "content": "SQL server connection parameters", "score": 0.85},
            {"id": "distant", "content": "Web server configuration", "score": 0.65},
        ]
    ...
    def test_semantic_indexer_error_recovery(self, semantic_indexer, mock_voyage_client):
        ...
        try:
            semantic_indexer.index_document_content(
                "Test content for error recovery", str(self.workspace / "test.md"), "markdown"
            )
        except Exception:
        ...
        semantic_indexer.index_document_content(
            "Recovery test content", str(self.workspace / "recovery.md"), "markdown"
        )
