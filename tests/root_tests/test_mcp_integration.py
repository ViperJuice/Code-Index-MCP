...
''')
    ...
    try:
        ...
        loaded_langs = dispatcher._loaded_languages if hasattr(dispatcher, '_loaded_languages') else set()
        ...
        for result in results[:3]:
            print(f"  - {result.get('file', 'N/A')}:{result.get('line', 'N/A')} - {result.get('snippet', 'N/A').strip()}")
