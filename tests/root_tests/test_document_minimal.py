...
def test_minimal():
    ...
    try:
        ...
        markdown_config = LANGUAGE_CONFIGS.get('markdown', {
            'code': 'markdown',
            'name': 'Markdown',
        ...
        plaintext_config = LANGUAGE_CONFIGS.get('plaintext', {
            'code': 'plaintext',
            'name': 'Plain Text',
...
"""
        ...
        if result:
            for i, symbol in enumerate(result[:3]):  # Show first 3
                print(f"  Symbol {i+1}: {symbol.get('name', 'unnamed')} ({symbol.get('type', 'unknown')})")
    except Exception as e:
        print(f"  ❌ Error: {e}")
        import traceback
        traceback.print_exc()
