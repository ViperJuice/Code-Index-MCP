...
async def test_cross_language_search(repo_paths: Dict[str, Path]) -> Dict:
    ...
    cross_lang_queries = ["error", "config", "parse", "handle", "create"]
    ...
    for query in cross_lang_queries:
        ...
        results[query] = {"total": len(search_results), "by_language": by_language}

        if search_results:
            print(
                f"   ✓ '{query}': {len(search_results)} results across {len(by_language)} languages"
            )
        else:
...
async def run_comprehensive_test():
    ...
    try:
        ...
        # Success criteria
        success = (
            successful_repos >= len(TEST_REPOSITORIES) * 0.8  # 80% repos working
            and total_files_indexed > 10  # At least 10 files indexed
            and total_symbol_results > 0  # Symbol search working
            and len(all_languages) >= 5  # At least 5 languages active
        )
