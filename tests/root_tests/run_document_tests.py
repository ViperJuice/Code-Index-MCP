...
class SimpleTestRunner:
    ...
    def run_test(self, test_file: str) -> TestResult:
        ...
        try:
            ...
            return TestResult(
                name=test_file, status="timeout", duration=60.0, message="Test timed out"
            )
        except Exception as e:
            return TestResult(
                name=test_file, status="error", duration=time.time() - start, message=str(e)
            )
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            for test_file, future in futures:
                try:
                    ...
                    print(
                        f"{icon} {Path(test_file).stem}: {result.message} ({result.duration:.2f}s)"
                    )

                except Exception as e:
                    print(f"❌ {Path(test_file).stem}: Error - {e}")
                    self.results.append(
                        TestResult(name=test_file, status="error", duration=0, message=str(e))
                    )
    ...
    def print_summary(self):
        ...
        report = {
            ...
            "results": [
                {"name": r.name, "status": r.status, "duration": r.duration, "message": r.message}
                for r in self.results
            ],
        }
