...
class RerankingQualityTester:
    ...
    async def test_reranking_effect(self):
        ...
        test_queries = [
            {
                ...
                "expected_files": ["auth_manager.py", "security_middleware.py", "gateway.py"],
            },
            {
                "query": "plugin system architecture",
                "expected_files": ["plugin_manager.py", "plugin_registry.py", "plugin_base.py"],
            },
            {"query": "error handling exception", "expected_files": ["errors.py", "dispatcher.py"]},
        ]
        ...
        for test_case in test_queries:
            ...
            tfidf_settings = RerankingSettings(
                enabled=True, reranker_type="tfidf", top_k=10, cache_ttl=0
            )
    ...
    async def test_reranker_initialization(self):
        ...
        # Test Cross-Encoder
        try:
            cross_encoder = factory.create_reranker(
                "cross-encoder", {"model": "cross-encoder/ms-marco-MiniLM-L-6-v2", "device": "cpu"}
            )
            result = await cross_encoder.initialize({})
            logger.info(
                f"Cross-Encoder initialization: {'SUCCESS' if result.is_success else 'FAILED'}"
            )
            if not result.is_success:
                logger.error(f"  Error: {result.error}")
            else:
    ...
    async def test_actual_reranking(self):
        ...
        baseline_search = HybridSearch(
            storage=self.storage, bm25_indexer=self.bm25_indexer, config=self.base_config
        )
        ...
        for r in baseline_results[:10]:
            search_results.append(
                ISearchResult(
                    ...
                    start_line=r.get("metadata", {}).get("line", 1),
                    end_line=r.get("metadata", {}).get("line", 1),
                    column=0,
        ...
        if rerank_result.is_success:
            ...
            for rr in rerank_result.data[:5]:
                ...
                logger.info(
                    f"  {new_idx+1}. {Path(filepath).name} (score: {score:.4f}, was #{orig_idx+1})"
                )
        else:
