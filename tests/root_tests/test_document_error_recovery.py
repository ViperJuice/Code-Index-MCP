    ...
    def test_concurrent_access_recovery(self, setup_plugins):
        ...
        for i in range(5):
            thread = threading.Thread(
                target=index_file, args=(plugins["markdown"], "concurrent.md", i)
            )
...
Also good content."""
        ...
        with patch.object(
            plugin.chunk_strategy, "create_chunks", side_effect=failing_chunk_creation
        ):
...
Final content."""
            ...
            if hasattr(plugin, "semantic_indexer"):
                ...
                with patch.object(
                    plugin.semantic_indexer, "generate_embeddings", side_effect=failing_embeddings
                ):
