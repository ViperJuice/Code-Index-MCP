...
from mcp_server.indexer.reranker import (
    RerankConfig, TFIDFReranker, LocalCrossEncoderReranker, 
    HybridReranker, CohereReranker
)
...
class RerankerDemo:
    ...
    async def test_reranking(self, query: str, reranker_type: str = "tfidf", top_k: int = 10):
        ...
        results_reranked = await self.search_engine.search(
            query,
            limit=top_k,
...
async def main():
    ...
    try:
        ...
        print(f"\nLanguage breakdown:")
        for ext, count in sorted(stats['language_breakdown'].items(), key=lambda x: x[1], reverse=True):
