...
class TestDocumentStorage(BaseDocumentTest):
    ...
    def test_document_metadata_storage(self, sqlite_store):
        ...
        structure = DocumentStructure(
            ...
            metadata={"author": "Test Author", "version": "1.0.0", "last_updated": "2024-01-01"},
        )
    ...
    def test_storage_concurrent_access(self, sqlite_store, temp_workspace):
        ...
        def index_subset(start, end):
            try:
                ...
                files = [str(temp_workspace / f"concurrent_{i}.md") for i in range(start, end)]
                for f in files:
                    if Path(f).exists():
                        dispatcher.index(f)
    ...
    def test_storage_compression(self):
        ...
        try:
            ...
            conn.execute(
                ...
                VALUES (?, ?, ?, ?)
            """,
                ("/large.md", compressed, len(large_content.encode()), len(compressed)),
            )
