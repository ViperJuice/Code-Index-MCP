...
async def test_mcp_cli_integration():
    ...
    try:
        ...
        from mcp_server_cli import call_tool, initialize_services
        ...
        # Test results
        test_results = {"initialization": True, "tools_tested": {}, "overall_success": True}
        ...
        try:
            ...
            print(
                f"   ✅ Languages Supported: {status_data.get('languages', {}).get('supported', 0)}"
            )
...
"""
        ...
        try:
            ...
            test_results["tools_tested"]["symbol_lookup"] = {
                "success": success,
                "result_preview": (
                    lookup_text[:100] + "..." if len(lookup_text) > 100 else lookup_text
                ),
            }
        ...
        successful_tools = sum(
            1 for t in test_results["tools_tested"].values() if t.get("success", False)
        )
