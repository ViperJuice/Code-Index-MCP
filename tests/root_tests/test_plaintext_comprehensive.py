...
Content with another underline style."""
        ...
        for p in paragraphs:
            if (
                ...
                and (p.text.isupper() or p.text.istitle())
            ):
...
class TestTopicExtractor:
    ...
    def test_phrase_extraction(self, extractor):
        ...
        found_phrases = sum(
            1 for exp in expected_phrases if any(exp in phrase for phrase in phrase_list)
        )
    ...
    def test_sentiment_analysis(self, extractor):
        ...
        positive_text = (
            "This is an excellent product with amazing features and outstanding performance."
        )
        negative_text = (
            "This is a terrible product with awful quality and horrible user experience."
        )
...
"""
    ...
    def test_structure_extraction(self, plugin, sample_text_file):
        ...
        found_sections = sum(
            1 for exp in expected_sections if any(exp in title for title in section_titles)
        )
...
"""
        ...
        chapter_sections = [
            s for s in structure.sections if "chapter" in s.get("title", "").lower()
        ]
...
"""
        ...
        found = sum(
            ...
            if any(exp.lower() in title.lower() for title in section_titles)
        )
        assert found >= 2
