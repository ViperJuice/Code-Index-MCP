...
from mcp_server.indexer.index_engine import (
    IndexEngine, IndexResult, BatchIndexResult, IndexOptions, 
    IndexProgress, IndexTask
)
from mcp_server.indexer.query_optimizer import (
    QueryOptimizer, Query, QueryType, QueryCost, OptimizedQuery,
    SearchPlan, IndexSuggestion, PerformanceReport, IndexType
)
...
class TestIndexEngine:
    ...
    def mock_storage(self):
        ...
        storage.get_statistics.return_value = {
            'files': 0, 'symbols': 0, 'symbol_references': 0
        }
        return storage
...
''')
    ...
    def test_get_progress(self, index_engine):
        ...
        index_engine._progress = IndexProgress(
            total=100,
            completed=50,
            failed=5
        )
