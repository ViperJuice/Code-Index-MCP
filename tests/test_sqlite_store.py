...
class TestDatabaseInitialization:
    ...
    def test_schema_tables_created(self, sqlite_store):
        ...
        expected_tables = [
            ...
            'imports', 'symbol_references', 'symbol_trigrams',
            'embeddings', 'query_cache', 'parse_cache', 'migrations'
        ]
    ...
    def test_indexes_created(self, sqlite_store):
        ...
        expected_indexes = [
            'idx_files_language', 'idx_files_hash',
            'idx_symbols_name', 'idx_symbols_kind', 'idx_symbols_file',
            'idx_imports_file', 'idx_imports_path',
            'idx_references_symbol', 'idx_references_file',
            'idx_trigrams', 'idx_embeddings_file', 'idx_embeddings_symbol',
            'idx_cache_expires'
        ]
...
class TestSymbolOperations:
    ...
    def test_store_symbol_creates_trigrams(self, sqlite_store):
        ...
        symbol_id = sqlite_store.store_symbol(
            file_id, "TestClass", "class", 1, 100
        )
        
        # Check trigrams created
        with sqlite_store._get_connection() as conn:
            cursor = conn.execute(
                "SELECT COUNT(*) FROM symbol_trigrams WHERE symbol_id = ?",
                (symbol_id,)
            )
    ...
    def test_symbol_fts_triggers(self, sqlite_store):
        ...
        # Store symbol
        symbol_id = sqlite_store.store_symbol(
            file_id, "search_me", "function", 1, 10,
...
class TestReferenceOperations:
    ...
    def test_store_reference(self, sqlite_store):
        ...
        symbol_id = sqlite_store.store_symbol(
            def_file_id, "helper", "function", 1, 5
        )
    ...
    def test_get_references(self, sqlite_store):
        ...
        assert len(refs) == 3
        assert {r['reference_kind'] for r in refs} == {"import", "instantiation", "inheritance"}
...
class TestSearchOperations:
    ...
    def test_fts_search_basic(self, sqlite_store):
        ...
        sqlite_store.store_symbol(
            file_id, "process_data", "function", 1, 10,
            documentation="Process incoming data from the API"
        )
        sqlite_store.store_symbol(
            file_id, "DataProcessor", "class", 20, 100,
            documentation="Main class for data processing operations"
        )
        sqlite_store.store_symbol(
            file_id, "validate_input", "function", 110, 120,
...
class TestPerformance:
    ...
    def test_fts_search_performance(self, populated_sqlite_store, benchmark_results):
        ...
        for i in range(50):
            populated_sqlite_store.store_symbol(
                file_id, f"documented_{i}", "function", i*10, i*10+5,
