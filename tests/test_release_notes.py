...
def test_changelog_has_rc1_section():
    ...
    assert re.search(r"^## \[1\.2\.0-rc1\] — \d{4}-\d{2}-\d{2}$", content, re.MULTILINE), \
...
def test_changelog_unreleased_empty():
    ...
    assert not re.search(r"^###\s", section_content, re.MULTILINE), \
...
def test_sandbox_has_p18_upgrade_link():
    ...
    assert "p18-upgrade.md" in content, \
        "sandbox.md missing link to p18-upgrade.md"
