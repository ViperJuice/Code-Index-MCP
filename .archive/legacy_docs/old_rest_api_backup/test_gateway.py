...
class TestGatewayStartupShutdown:
    ...
    def test_startup_success(self, mock_watcher, mock_dispatcher, mock_plugin, mock_store, test_client):
...
class TestSymbolEndpoint:
    ...
    def test_symbol_lookup_success(self, test_client_with_dispatcher, sample_symbol_def):
        ...
        test_client_with_dispatcher.app.state.dispatcher.lookup = Mock(return_value=sample_symbol_def)
...
class TestPluginsEndpoint:
    ...
    def test_plugins_list(self, test_client_with_dispatcher):
        ...
        test_client_with_dispatcher.app.state.dispatcher._plugins = [mock_py_plugin, mock_js_plugin]
...
class TestReindexEndpoint:
    ...
    async def test_reindex_specific_file(self, test_client_with_dispatcher, temp_code_directory):
        ...
        test_client_with_dispatcher.app.state.dispatcher.index_file.assert_called_once_with(file_path)
...
class TestPerformance:
    ...
    def test_search_performance(self, test_client_with_dispatcher, benchmark_results):
        ...
        test_client_with_dispatcher.app.state.dispatcher.search = Mock(
            return_value=[SearchResult(name=f"result_{i}", kind="function", path=f"/file{i}.py", score=1.0) for i in range(20)]
        )
